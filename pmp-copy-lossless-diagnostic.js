(() => {
  ...
  async function diagnose(stage){
    ...
    const report={type:'PMP_LOSSLESS_QUALITY_DIAGNOSTIC',version:'3.1.0-freshness-proof',checked_at:new Date().toISOString(),stage:stage||'manual',overall,local_app:{status:localStatus,passed:localPass,weak_spots:localWeak},github_vault:{status:vaultStatus,passed:vaultPass,weak_spots:vaultWeak,files:vaultResults,freshness,note:vaultStatus==='GOOD'?'Latest files match the most recent recorded save.':'Save freshness is not proven yet.'},lossless_quality:{status:qualityStatus,passed:qualityPass,weak_spots:qualityWeak},next:overall==='GOOD'?'Safe to use Save to GitHub Vault.':'Fix WATCH sections, or press Save to GitHub Vault and run this check again after GitHub updates.',previous_visible_status:statusText,copy_button_found:!!copyButton,copy_button_text:copyButton,copyCurrent_exists:!!(w&&typeof w.copyCurrent==='function'),copyLosslessReport_exists:!!(w&&typeof w.copyLosslessReport==='function'),inventory_eyes_key_exists:!!eyes,inventory_key_exists:!!inv,packet_can_build:!!localReport,shortcut_url:SHORTCUT_URL};
  ...
  function patchImproveButton(d){for(const b of Array.from(d.querySelectorAll('button'))){const t=(b.textContent||'').replace(/\s+/g,' ').trim();if(t.includes('Improve Lossless Quality')){b.dataset.losslessDiagCopyImprove='5';b.onclick=function(e){if(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation()}copyDiagnostic('improve_lossless_quality_button_direct');return false};if(!b.dataset.losslessDiagCopyImproveCapture5){b.dataset.losslessDiagCopyImproveCapture5='1';b.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation();copyDiagnostic('improve_lossless_quality_button_capture');return false},true)}}}}
