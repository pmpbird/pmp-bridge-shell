(()=>{
'use strict';
const V='1.6.0-pass7-cachelift-force-latest-frame-loader';
...
const LATEST_SRC='pmp-continuous-run-bank-order-frame-loader-v1.js?fresh=pass7-runtime-probe-loader-forced-20260706L';
const RULE='Cachelift bridge only. Forces latest Pass 7 frame loader into the current iframe when an old hardcoded loader is present. No route change, no storage migration, no Bank rebuild, no ownership takeover.';
...
function injectDoc(d){let events=[];if(!d||!d.body)return{made:0,events:['no_body']};try{let old=d.getElementById(LOADER_ID);if(old){let src=old.getAttribute('src')||'';if(src.indexOf('pass7-runtime-probe-loader-forced-20260706L')>=0)return{made:0,events:['latest_loader_already_present']};events.push('removed_old_loader:'+src);try{old.remove()}catch(e){events.push('remove_old_loader_error:'+String(e&&e.message||e))}}
let s=d.createElement('script');s.id=LOADER_ID;s.src=LATEST_SRC+'-'+Date.now();s.onload=()=>{try{put({type:'PMP_MOUNT_REGISTRY_CACHELIFT_LOADER_FORCE_RECEIPT_V1',version:V,owner:OWNER,at:now(),event:'latest_loader_loaded',src:s.src,rule:RULE})}catch(e){}};s.onerror=()=>{try{put({type:'PMP_MOUNT_REGISTRY_CACHELIFT_LOADER_FORCE_RECEIPT_V1',version:V,owner:OWNER,at:now(),event:'latest_loader_error',src:s.src,rule:RULE})}catch(e){}};d.body.appendChild(s);events.push('latest_loader_injected:'+s.src);return{made:1,events}}catch(e){events.push('inject_exception:'+String(e&&e.message||e));return{made:0,events}}}
