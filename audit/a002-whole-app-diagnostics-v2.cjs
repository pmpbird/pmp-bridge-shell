...
function writeOutput() {
  const output = {
    type: 'PMP_A002_WHOLE_APP_DIAGNOSTICS_CERTIFICATION_V2',
...
async function waitForFrame(page, pattern, timeout = 45000) {
...
async function enterCurrentApp(page) {
  // Diagnostics is an internal home-surface tab, not a Current Map route.
  // The canonical outer route is #control; the Diagnostics owner renders inside it.
  ...
  const home = await waitForFrame(page, /pmp-home-single-v6\.html/, 45000);
  record('current-chain-home-frame-ready', !!home, { url: home.url(), page_url: page.url() });
  ...
  try {
    ...
    const state = await page.evaluate(async ({ requiredBootVersion, requiredBcdVersion, sections }) => {
      ...
      const runResult = bootstrapApi && typeof bootstrapApi.run === 'function'
        ? await bootstrapApi.run('a002_whole_app_diagnostics_certification_v2')
        : null;
    ...
    record('current-app-canonical-control-route',
      new URL(state.pageUrl).pathname.endsWith('/' + CURRENT) && new URL(state.pageUrl).hash === '#control',
