...
function writeOutput() {
  const output = {
    type: 'PMP_A002_WHOLE_APP_DIAGNOSTICS_CERTIFICATION_V4',
  ...
  console.log(`A002_DIAGNOSTICS_RESULT_WRITTEN ${RESULT_PATH} ${JSON.stringify({tests_total:output.tests_total,tests_passed:output.tests_passed,tests_failed:output.tests_failed,runtime_errors:output.runtime_errors.length,fatal_error:!!fatalError})}`);
}
async function waitForFrame(page, pattern, timeout = 60000) {
...
async function waitForApiFrame(page, name, timeout = 60000) {
  const deadline = Date.now() + timeout;
  while (Date.now() < deadline) {
    for (const frame of page.frames()) {
      try {
        const ready = await frame.evaluate(apiName => {
          const value = window[apiName];
...
async function retiredTraceControls(page) {
  const found = [];
  for (const frame of page.frames()) {
    try {
      const count = await frame.locator('#pmpWholeAppHealthLayoutTraceV1, [data-pmp-whole-app-health-layout-trace]').count();
...
(async () => {
  ...
  try {
    ...
    record('current-inner-v30-diagnostics-owner-frame-ready',
      inner.url().includes(INNER) && apiFrame.url().includes(INNER),
      { outer_url: page.url(), inner_url: inner.url(), api_frame_url: apiFrame.url(), frame_urls: page.frames().map(frame => frame.url()) }
    );

    const state = await apiFrame.evaluate(async ({ requiredBcdVersion, sections }) => {
      const parse = key => {
        try { return JSON.parse(localStorage.getItem(key) || 'null'); }
        catch { return null; }
      };
      const api = window.PMPCurrentBCDDiagnosticsBootstrapV1;
      const runResult = await api.run('a002_whole_app_diagnostics_certification_v4');
      return {
    ...
    }, { requiredBcdVersion: REQUIRED_BCD_VERSION, sections: SECTIONS });
    ...
    for (const section of SECTIONS) {
      ...
      record(`whole-app-section-pass:${section}`, value?.status === 'PASS', {
        status: value?.status || 'MISSING', type: value?.type || null,
    ...
    record('continuous-run-canonical-32-level-proof',
      state.continuousReceipt?.status === 'PASS' && state.continuousReceipt?.expected_level_count === 32 && Array.isArray(state.continuousReceipt?.expected_levels) && state.continuousReceipt.expected_levels.length === 32,
      { continuous: state.continuousReceipt }
    );

    const traceControls = await retiredTraceControls(page);
    record('retired-layout-trace-control-absent', traceControls.length === 0, traceControls);
    const relevantErrors = runtimeErrors.filter(error => /diagnostic|passes[- _]?bcd|journal|continuous|syntaxerror|pmp-diagnostic-coverage-passes-bcd-v1-1-1-0/i.test(String(error.message || '') + ' ' + String(error.stack || '')));
    record('no-diagnostics-runtime-errors', relevantErrors.length === 0, relevantErrors);

    if (results.some(row => !row.pass)) throw new Error('Whole App diagnostics certification failed');
  } catch (error) {
    ...
    catch (error) { if (!fatalError) fatalError = errorValue(error); process.exitCode = 1; }
    try { writeOutput(); }
