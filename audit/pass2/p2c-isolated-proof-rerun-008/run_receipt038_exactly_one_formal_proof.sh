...
seal_path=root/'P2C_REHEARSAL_EQUIVALENCE_SEAL_RECEIPT_037.json'
...
assert auth['equivalence_seal_git_blob_sha']==directive['equivalence_seal_git_blob_sha']==blob(seal_path)
...
changed=set(subprocess.check_output(['git','diff','--name-only',directive['current_main_sha'],os.environ['PR_HEAD_SHA']],text=True).splitlines())
...
(pathlib.Path(os.environ['EVIDENCE_DIR'])/'receipt038-execution-binding-check.json').write_text(json.dumps({
  ...
  'base_sha':os.environ['BASE_SHA'],'pr_head_sha':os.environ['PR_HEAD_SHA'],'checkout_head':checkout_head,
  'proof_run_count_authorized':1,'proof_run_count_previously_executed':0,'second_proof_run_authorized':False,
