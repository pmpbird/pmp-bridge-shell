...
OLD_RUNTIME_RECEIPT_SHA256='0bbd7c4bcace7101237b9a0b8b612bf90bebd9697b95efc90a61bb6cdf438f26'
NEW_RUNTIME_RECEIPT_SHA256='928f551d28ce57a2ffd30c533503448685a155813ae02f805b164e6483fdc4d0'
...
python3 - "$MATERIALIZED_CONTROLLER" "$OLD_PATCHER_SHA256" "$NEW_PATCHER_SHA256" "$OLD_MANIFEST_SHA256" "$NEW_MANIFEST_SHA256" "$OLD_CONTROLLER" "$NEW_CONTROLLER" "$OLD_CONTROLLER_SHA256" "$NEW_CONTROLLER_SHA256" "$OLD_RUNTIME_RECEIPT_SHA256" "$NEW_RUNTIME_RECEIPT_SHA256" "$OLD_ALLOWLIST" "$NEW_ALLOWLIST" <<'PY'
import pathlib,sys
path=pathlib.Path(sys.argv[1])
old_patcher,new_patcher,old_manifest,new_manifest,old_controller,new_controller,old_controller_sha,new_controller_sha,old_runtime_sha,new_runtime_sha,old_allowlist,new_allowlist=sys.argv[2:]
text=path.read_text()
...
expected_counts={old_patcher:1,old_manifest:1,old_controller:4,old_controller_sha:1,old_runtime_sha:1,old_allowlist:1,old_install:1}
labels={old_patcher:'PATCHER_SHA',old_manifest:'MANIFEST_SHA',old_controller:'CONTROLLER_PATH',old_controller_sha:'CONTROLLER_SHA',old_runtime_sha:'RUNTIME_RECEIPT_SHA',old_allowlist:'INTERNAL_ALLOWLIST',old_install:'BROWSER_INSTALL_CWD'}
...
text=text.replace(old_runtime_sha,new_runtime_sha,1)
...
if text.count(new_runtime_sha)!=1:raise SystemExit(f'RECEIPT024_NEW_RUNTIME_RECEIPT_SHA_COUNT_INVALID:{text.count(new_runtime_sha)}')
...
PY
if [ "${P2C_STOP_AFTER_PREPARATION:-0}" = "1" ]; then
  python3 - "$MATERIALIZED_CONTROLLER" <<'PY'
import pathlib,sys
path=pathlib.Path(sys.argv[1])
text=path.read_text()
marker='echo "=== Run exactly one active browser rollback and restored regression chain ==="'
if text.count(marker)!=1:raise SystemExit(f'PROOF_MARKER_COUNT_INVALID:{text.count(marker)}')
prefix=text.split(marker,1)[0]
path.write_text(prefix+"printf '%s\\n' 'P2C_RUNTIME_BINDING_HARD_STOP_AFTER_PREPARATION'\n")
PY
fi
