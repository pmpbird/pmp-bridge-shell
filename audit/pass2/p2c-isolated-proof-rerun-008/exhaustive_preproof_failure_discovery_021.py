...
def issue(q,order,label,path,expected,actual,command,line,dependency): q.append({'dependency_order':order,'label':label,'affected_file':str(path),'expected_anchor_or_condition':expected,'actual_match_count_or_value':actual,'exact_command':command,'controller_line':line,'dependency':dependency})
def main():
 ...
 anchors=[(1,'R009C002_GATE_PROPERTY_MAP',prepare,' g=g.replace("const tokenSet=new WeakSet();","const tokenSet=new WeakSet();\\nconst tokenValidators=new WeakMap();",1)',27),(2,'R009C002_GATE_AUTHORITY_FAILURE_SETTLEMENT_ANCHOR',prepare,' anchor="function wrapCallback(token,callback){return typeof callback===\'function\'?function(){return run(token,callback,this,Array.from(arguments))}:callback}\\n"',31),(3,'R009C002_GATE_AUTHORITY_FAILURE_SETTLEMENT_REPLACE',prepare,' g=g.replace(anchor,new_validator+anchor,1)',34),(4,'R009C002_PREPARE_GATE_EXTENSION',prepare," gate_path.write_text(g)\n return {'status':'APPLIED','authority_model':'CALLBACK_BOUND_TOKEN_VALIDATOR','global_ambient_authority':False,'lease_revalidated_on_callback':True,'ambient_depth_constant_zero':True}",38),(5,'R009C002_PREPARE_MAIN_ANCHOR',prepare,'def main():\n',86),(6,'R009C002_NORMALIZED_SOURCE_APPLICATION',prepare," for p in sorted((a.payload_root/'contracts').iterdir()):shutil.copy2(p,a.activated_root/p.name)\n # Proof-only repair: prefetch all source bytes before installing the gate; no production file is modified.",109),(7,'R009C002_PREPARE_OUTPUT',prepare,"'callback_bound_actor_lease_repair':callback_bound_actor_lease_repair,'production_changed':False",113),(8,'R009C002_A002_DETERMINISM_PATCH',runner,' s=s.replace(home_anchor,home_insert,1)\n a002.write_text(s)',121)]
 ...
 policy=json.loads(policy_path.read_text());actors={x['path']:x for x in policy.get('actors',[])};compat=[]
 for i,row in enumerate(manifest['records'],1):
  ...
  elif actor.get('sha256')!=row['original_sha256']:
   allowed=row['path']=='pmp-pass75-reload-runtime-platform-gate-v1.js' and actor.get('sha256')=='3bd54e89efc2cadfb89c55cf4811c11e176dcd6c43d5402d62a0da832772acb5' and row['original_sha256']=='203e1d0096f517e66b06f0534d991ab54555f936a5d2be42d9e8b2fa57fd5042'
   if allowed: compat.append({'path':row['path'],'frozen_policy_sha256':actor.get('sha256'),'current_original_sha256':row['original_sha256']})
   else: issue(q,40+i,'R009C002_POLICY_ORIGINAL_SHA_MISMATCH',policy_path,row['original_sha256'],actor.get('sha256'),'update_policy()',174,'policy source identity')
 ...
 superseded={'efe861570ea5d8e63e197d468bee287ca179f849938973b1ed977658fb75af57':'old patcher','72d664d3ddf1890da802a5b7e00138a487682fde9859ef1c036f05c69e840619':'old manifest','fb87f6dfc4a46cc07927b9ba78e1f8ac657eed2d16b4764b63f5ecb8c787e238':'intermediate manifest'};historical=[]
 for value,meaning in superseded.items():
  ...
  for loc in p.stdout.splitlines():historical.append({'superseded_value':value,'meaning':meaning,'location':loc,'classification':'HISTORICAL_RECEIPT_OR_EXPLICIT_OLD_TO_NEW_BINDING'})
 q=sorted(q,key=lambda x:(x['dependency_order'],x['label']))
 out={'type':'PMP_PASS2_EXHAUSTIVE_PREPROOF_FAILURE_DISCOVERY_021','status':'PASS_NO_REMAINING_PREPROOF_FAILURES' if not q else 'COMPLETE_FAILURE_SET_COLLECTED','repair_queue_count':len(q),'repair_queue_dependency_ordered':q,'anchor_checks':checks,'record6_policy_compatibility':compat,'controller_executed_diagnostic_mode':controller_ran,'controller_exit_code':crc,'controller_stdout':cout[-4000:],'controller_stderr':cerr[-4000:],'compile_results':comp,'historical_checksum_references':historical,'hard_stop':{'git_worktree_add_executed':False,'playwright_or_chromium_install_executed':False,'disposable_copy_preparation_executed':False,'browser_proof_executed':False,'proof_run_count_executed':0,'proof_authorized':False,'production_files_modified':False,'current_map_modified':False,'persisted_data_modified':False}}
