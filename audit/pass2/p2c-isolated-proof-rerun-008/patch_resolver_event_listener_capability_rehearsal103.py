...
CAPABILITY = 'event_listener'
EXPECTED_TEMPLATE_TYPE = 'PMP_P2C_PRODUCTION_ENFORCEMENT_POLICY_CANDIDATE_002'
EXPECTED_BEFORE = ['network_fetch', 'timer_schedule']
EXPECTED_AFTER = ['network_fetch', 'timer_schedule', 'event_listener']
...
if not policy_path.is_file():
    raise SystemExit(f'REHEARSAL103_POLICY_NOT_FOUND:{policy_path}')
...
if policy.get('type') != EXPECTED_TEMPLATE_TYPE:
    raise SystemExit(f"REHEARSAL103_POLICY_TYPE_INVALID:{policy.get('type')}")
if policy.get('unknown_actor_policy') != EXPECTED_FAIL_CLOSED:
    raise SystemExit(f"REHEARSAL103_UNKNOWN_ACTOR_POLICY_INVALID:{policy.get('unknown_actor_policy')}")
if policy.get('unauthorized_capability_policy') != EXPECTED_FAIL_CLOSED:
    raise SystemExit(f"REHEARSAL103_UNAUTHORIZED_CAPABILITY_POLICY_INVALID:{policy.get('unauthorized_capability_policy')}")

actors = policy.get('actors')
if not isinstance(actors, list):
    raise SystemExit('REHEARSAL103_ACTOR_LIST_INVALID')
matches = [actor for actor in actors if actor.get('path') == RESOLVER]
if len(matches) != 1:
    raise SystemExit(f'REHEARSAL103_RESOLVER_ACTOR_COUNT_INVALID:{len(matches)}')
resolver = matches[0]
capabilities_before = resolver.get('capabilities')
if capabilities_before != EXPECTED_BEFORE:
    raise SystemExit('REHEARSAL103_RESOLVER_CAPABILITIES_BEFORE_INVALID:' + json.dumps(capabilities_before, sort_keys=True))
...
if changed != [RESOLVER]:
    raise SystemExit('REHEARSAL103_CHANGED_ACTOR_SET_INVALID:' + json.dumps(changed, sort_keys=True))
if len(before['actors']) != len(policy['actors']):
    raise SystemExit('REHEARSAL103_ACTOR_COUNT_CHANGED')
if policy.get('type') != before.get('type'):
    raise SystemExit('REHEARSAL103_POLICY_TYPE_CHANGED')
if policy.get('unknown_actor_policy') != before.get('unknown_actor_policy'):
    raise SystemExit('REHEARSAL103_UNKNOWN_ACTOR_POLICY_CHANGED')
if policy.get('unauthorized_capability_policy') != before.get('unauthorized_capability_policy'):
    raise SystemExit('REHEARSAL103_UNAUTHORIZED_CAPABILITY_POLICY_CHANGED')
...
if len(resolver_after) != 1 or resolver_after[0].get('capabilities') != EXPECTED_AFTER:
    raise SystemExit('REHEARSAL103_RESOLVER_CAPABILITY_WRITE_VERIFICATION_FAILED')
if verification.get('type') != EXPECTED_TEMPLATE_TYPE:
    raise SystemExit('REHEARSAL103_POLICY_TYPE_WRITE_VERIFICATION_FAILED')
if verification.get('unknown_actor_policy') != EXPECTED_FAIL_CLOSED:
    raise SystemExit('REHEARSAL103_UNKNOWN_ACTOR_POLICY_WRITE_VERIFICATION_FAILED')
if verification.get('unauthorized_capability_policy') != EXPECTED_FAIL_CLOSED:
    raise SystemExit('REHEARSAL103_UNAUTHORIZED_CAPABILITY_POLICY_WRITE_VERIFICATION_FAILED')

unchanged_actor_count = sum(
    1 for old_actor, new_actor in zip(before['actors'], verification['actors'])
    if old_actor == new_actor
)
if unchanged_actor_count != len(before['actors']) - 1:
    raise SystemExit(f'REHEARSAL103_UNCHANGED_ACTOR_COUNT_INVALID:{unchanged_actor_count}')

evidence = {
    'type': 'PMP_P2C_RESOLVER_EVENT_LISTENER_CAPABILITY_REPAIR_103',
    'status': 'PASS',
    ...
    'unchanged_actor_count': unchanged_actor_count,
    ...
    'required_runtime_operation': {
        'operation': 'MessageChannel.port1.onmessage registration',
        'event_type': 'message',
        'purpose': 'integrity_worker_status_response',
    },
...
(evidence_dir / 'resolver-event-listener-capability-repair-103.json').write_text(
