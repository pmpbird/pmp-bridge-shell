...
auth_path=root/'P2C_SOURCE_COMMIT_CONSTANT_REPAIR_AUTHORIZATION_RECEIPT_078.json'
directive_path=root/'P2C_RECEIPT078_EXACTLY_ONE_FORMAL_PROOF_EXECUTION_DIRECTIVE_079.json'
seal_path=root/'P2C_RECEIPT078_FORMAL_PROOF_PR_HEAD_SEAL_080.json'
...
assert auth['source_commit_patcher_git_blob_sha']==blob(pathlib.Path(auth['source_commit_patcher_path']))
...
PY
bash -n "$AUDIT_DIR/rerun008-controller-main-receipt078.sh"
bash -n "$AUDIT_DIR/rerun008-controller-finalize.sh"
bash "$AUDIT_DIR/rerun008-controller-main-receipt078.sh"
