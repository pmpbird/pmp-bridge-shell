...
auth_path=root/'P2C_PREPARE_RECEIPT_BINDING_EXACTLY_ONE_FORMAL_PROOF_AUTHORIZATION_RECEIPT_062.json'
directive_path=root/'P2C_RECEIPT062_EXACTLY_ONE_FORMAL_PROOF_EXECUTION_DIRECTIVE_063.json'
...
assert blob(auth['prepare_proxy_path'])==auth['prepare_proxy_git_blob_sha']==directive['prepare_proxy_git_blob_sha']
...
out={'type':'PMP_P2C_RECEIPT062_FORMAL_PROOF_AUTHORIZATION_STATIC_PREFLIGHT_RESULT_065','status':'PASS_STATIC_PREFLIGHT_ONLY','head_sha':head,'base_sha':base,'authorization_receipt':'062','execution_directive':'063','authorization_sha256':sha(auth_path),'authorization_git_blob_sha':blob(auth_path),'execution_wrapper_git_blob_sha':blob(directive['execution_wrapper_path']),'formal_workflow_git_blob_sha':blob(auth['formal_workflow_path']),'formal_controller_wrapper_git_blob_sha':blob(auth['formal_wrapper_path']),'prepare_proxy_git_blob_sha':blob(auth['prepare_proxy_path']),'formal_finalizer_git_blob_sha':blob(auth['formal_finalizer_path']),'proof_run_count_executed':0,'formal_proof_executed':False,'merge_authorized':False,'production_activation_authorized':False,'current_map_change_authorized':False,'persisted_data_change_authorized':False,'second_proof_run_authorized':False}
pathlib.Path(os.environ['EVIDENCE_DIR']).mkdir(parents=True,exist_ok=True)
(pathlib.Path(os.environ['EVIDENCE_DIR'])/'static-preflight-result-065.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
