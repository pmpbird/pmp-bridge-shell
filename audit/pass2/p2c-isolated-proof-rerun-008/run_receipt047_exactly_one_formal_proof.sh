...
head_seal_path=root/'P2C_RECEIPT047_FORMAL_PROOF_PR_HEAD_SEAL_049.json'
...
(pathlib.Path(os.environ['EVIDENCE_DIR'])/'receipt047-execution-binding-check.json').write_text(json.dumps({'status':'PASS','authorization_receipt_sha256':sha(auth_path),'directive_sha256':sha(directive_path),'base_sha':os.environ['BASE_SHA'],'pr_head_sha':pr_head,'checkout_head':checkout_head,'sealed_parent_commit':head_seal['sealed_parent_commit'],'proof_run_count_authorized':1,'proof_run_count_previously_executed':0,'second_proof_run_authorized':False,'production_activation_authorized':False,'merge_authorized':False},indent=2,sort_keys=True)+'\n')
PY
