...
def transform_overlay(closure:zipfile.ZipFile,root:Path,receipt_sha:str):
    ...
    for n in brokers:(root/n).write_bytes(closure.read(n))
    policy=json.loads(closure.read("pmp-p2c-production-enforcement-policy-candidate-002.json"))
    ...
    manifest=json.loads(closure.read("pmp-p2c-production-enforcement-source-manifest-candidate-002.json"))
    ...
    reg=json.loads(closure.read("pmp-p2c-production-enforcement-broker-registry-candidate-002.json"))
    ...
    for n in ["P2C_STORAGE_OWNER_NAMESPACE_CONTRACT_001.json","P2C_INDEXEDDB_OWNER_NAMESPACE_CONTRACT_001.json","P2C_CACHE_OWNER_NAMESPACE_CONTRACT_001.json","P2C_VERIFIED_LOADER_OWNER_NAMESPACE_CONTRACT_001.json"]:(root/n).write_bytes(closure.read(n))
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument("--baseline-root",type=Path,required=True);ap.add_argument("--activated-root",type=Path,required=True);ap.add_argument("--patch-package",type=Path,required=True);ap.add_argument("--closure-package",type=Path,required=True);ap.add_argument("--authorization-receipt",type=Path,required=True);ap.add_argument("--output",type=Path,required=True);ap.add_argument("--activated-existing",action="store_true");x=ap.parse_args()
    if hf(x.patch_package)!=EXPECTED_PATCH_SHA:raise SystemExit("PATCH_PACKAGE_SHA_MISMATCH")
    if hf(x.closure_package)!=EXPECTED_CLOSURE_SHA:raise SystemExit("CLOSURE_PACKAGE_SHA_MISMATCH")
    ...
    with zipfile.ZipFile(x.patch_package) as pz:
        ...
        for op in pm["operations"]:
            ...
            data=pz.read("prospective/after/"+op["path"])
            if h(data)!=op["after_sha256"]:raise SystemExit("AFTER_IMAGE_MISMATCH:"+op["path"])
            p.parent.mkdir(parents=True,exist_ok=True);p.write_bytes(data)
    with zipfile.ZipFile(x.closure_package) as cz:transform_overlay(cz,x.activated_root,authsha)
