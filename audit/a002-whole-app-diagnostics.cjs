...
function writeOutput() {
  const output = {
    type: 'PMP_A002_WHOLE_APP_DIAGNOSTICS_CERTIFICATION_V1',
...
async function waitForFrame(page, pattern, timeout = 35000) {
...
async function enterCurrentDiagnostics(page) {
  await page.goto(BASE + 'pmp-app-current.html#diagnostics', { waitUntil: 'domcontentloaded' });
  ...
  await page.waitForURL(url => url.pathname.endsWith('/' + CURRENT) && url.hash === '#diagnostics', { timeout: 40000 });
  await waitForFrame(page, /pmp-home-single-v6\.html/, 40000);
  ...
  // Let the complete nested app and its declared owner/helper scripts settle before
  // requesting the final current-session proof.
  await page.waitForTimeout(5000);
}
async function countRetiredTraceControls(page) {
  const found = [];
...
(async () => {
  ...
  try {
    ...
    page.on('pageerror', error => runtimeErrors.push({ type: 'pageerror', ...serializableError(error) }));
    page.on('console', message => {
      if (message.type() === 'error') runtimeErrors.push({ type: 'console', message: message.text() });
    });

    await enterCurrentDiagnostics(page);
    ...
    const state = await page.evaluate(async ({ requiredBootVersion, requiredBcdVersion, sections }) => {
      ...
      const runResult = bootstrapApi && typeof bootstrapApi.run === 'function'
        ? await bootstrapApi.run('a002_whole_app_diagnostics_certification')
        : null;
      const bootstrapReceipt = parse('pmp_current_bcd_diagnostics_bootstrap_v1_receipt');
      const bcdReceipt = parse('pmp_diagnostic_coverage_passes_bcd_v1_receipt');
      const journalReceipt = parse('pmp_diagnostic_journal_v1_receipt');
      const journalViewReceipt = parse('pmp_diagnostic_journal_view_v1_receipt');
      const continuousReceipt = parse('pmp_continuous_run_canonical_level_receipt_v1');
      return {
        requiredBootVersion,
        requiredBcdVersion,
        sections,
    ...
    const completeSections = Array.isArray(state.bootstrapReceipt?.complete_sections)
    ...
    record('transactional-bootstrap-published-complete-current-receipt',
      ...
      SECTIONS.every(section => completeSections.includes(section)),
      { runResult: state.runResult, bootstrapReceipt: state.bootstrapReceipt }
    );
    ...
    record('diagnostic-journal-current-session-proof',
      ...
      { journal: state.journalReceipt, journalView: state.journalViewReceipt }
    );
    ...
    const retiredControls = await countRetiredTraceControls(page);
    record('retired-layout-trace-control-absent', retiredControls.length === 0, retiredControls);
    ...
    try { if (browser) await browser.close(); }
      ...
      if (!fatalError) fatalError = serializableError(error);
      process.exitCode = 1;
    }
    try { writeOutput(); }
    catch (error) {
