...
function writeOutput(){const output={type:'PMP_A002_WHOLE_APP_DIAGNOSTICS_CERTIFICATION_V5',generated_at:new Date().toISOString(),base_url:BASE,required_bootstrap_version:REQUIRED_BOOT_VERSION,required_bcd_version:REQUIRED_BCD_VERSION,required_sections:SECTIONS,tests_total:results.length,tests_passed:results.filter(x=>x.pass).length,tests_failed:results.filter(x=>!x.pass).length,runtime_errors:runtimeErrors,fatal_error:fatalError,results};fs.writeFileSync(RESULT_PATH,JSON.stringify(output,null,2));console.log(`A002_DIAGNOSTICS_RESULT_WRITTEN ${RESULT_PATH} ${JSON.stringify({tests_total:output.tests_total,tests_passed:output.tests_passed,tests_failed:output.tests_failed,runtime_errors:output.runtime_errors.length,fatal_error:!!fatalError})}`)}
...
async function enterCurrentApp(page){
  ...
  const gateReceiptBefore=await page.evaluate(()=>{try{return JSON.parse(localStorage.getItem('pmp_pass75_reload_runtime_platform_gate_v1_receipt')||'null')}catch{return null}});
  record('runtime-platform-certified-before-governed-entry',gateReceiptBefore?.certified===true&&gateReceiptBefore?.released===false,{receipt:gateReceiptBefore});
...
(async()=>{let browser=null;try{
  ...
  record('current-inner-v30-diagnostics-owner-frame-ready',inner.url().includes(INNER)&&apiFrame.url().includes(INNER),{outer_url:page.url(),inner_url:inner.url(),api_frame_url:apiFrame.url(),frame_urls:page.frames().map(f=>f.url())});

  const state=await apiFrame.evaluate(async({requiredBcdVersion,sections})=>{const parse=k=>{try{return JSON.parse(localStorage.getItem(k)||'null')}catch{return null}};const api=window.PMPCurrentBCDDiagnosticsBootstrapV1;const runResult=await api.run('a002_whole_app_diagnostics_certification_v5');return{apiVersion:api.version||null,runResult,bootstrapReceipt:parse('pmp_current_bcd_diagnostics_bootstrap_v1_receipt'),bcdReceipt:parse('pmp_diagnostic_coverage_passes_bcd_v1_receipt'),journalReceipt:parse('pmp_diagnostic_journal_v1_receipt'),journalViewReceipt:parse('pmp_diagnostic_journal_view_v1_receipt'),continuousReceipt:parse('pmp_continuous_run_canonical_level_receipt_v1'),requiredBcdVersion,sections}},{requiredBcdVersion:REQUIRED_BCD_VERSION,sections:SECTIONS});

  record('transactional-bootstrap-api-current',state.apiVersion===REQUIRED_BOOT_VERSION,{observed:state.apiVersion,required:REQUIRED_BOOT_VERSION});
