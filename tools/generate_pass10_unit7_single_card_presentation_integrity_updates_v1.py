...
OLD_FRESH = (
    "pmp-continuous-run-level-ui-scope-v1.js?"
    "fresh=pass10-unit7-legacy-alias-single-stack-20260727A"
)
NEW_FRESH = (
    "pmp-continuous-run-level-ui-scope-v1.js?"
    "fresh=pass10-unit7-single-card-presentation-20260727A"
)
...
def main() -> None:
    ...
    seal.update(
        ...
        sealed_branch="agent/pass10-unit7-single-card-visual-cleanup-v1",
        pass10_context=(
            "P10-U7T makes each Level 3 through 30B canonical shell present as "
            "one visual card like Levels 1 and 2. It keeps every legacy "
            "functional member, selector, control, readiness gate, hidden "
            "mount sentinel, and event-driven ownership node in place; only "
            "the redundant inner wrapper and repeated direct level heading "
            "are visually contained. The Pass 9 Locked / waiting fail-closed "
            "readiness state is preserved. The repair does not read, write, "
            "delete, migrate, or activate persisted data."
        ),
    )
    SEAL.write_text(
    ...
    print("PASS: P10-U7T single-card presentation runtime integrity identities regenerated")
