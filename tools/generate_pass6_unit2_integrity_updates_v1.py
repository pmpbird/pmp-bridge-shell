...
def main() -> None:
    ...
    seal.update(
        ...
        sealed_branch="agent/pass6-unit2-readonly-journal-view-export-v1",
        pass6_context=(
            "Pass 6 Unit 2 production-loads the pure journal contract and a read-only "
            "view/export adapter, then adds a Diagnostics card backed by an ephemeral "
            "empty journal. The owner has no append or restore path; export is manual, "
            "redacted, deterministic, and bounded to 256 entries and 262144 bytes."
        ),
    )
    ...
    print("PASS: Pass 6 Unit 2 integrity identities regenerated")
