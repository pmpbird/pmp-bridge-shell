...
VIEW_TAG='<script src="pmp-mount-lifecycle-diagnostics-view-v1.js?fresh=pass5-unit4-readonly-diagnostics-20260726A"></script>'
AUTHORITY_TAG='<script src="pmp-authority-rules-v1.js?fresh=bug-authority-phase2a-final-exclusive-owner"></script>'
OLD_BLOCK=RUNTIME_TAG+'\n'+AUTHORITY_TAG
NEW_BLOCK=RUNTIME_TAG+'\n'+VIEW_TAG+'\n'+AUTHORITY_TAG
...
def main():
    ...
    for path in (VIEW,DIAGNOSTICS,INNER_REL):
    ...
    seal.update(
        ...
        sealed_branch='agent/pass5-unit4-readonly-diagnostics-v1',
        pass5_context='Pass 5 Unit 4 adds one redacted read-only lifecycle Diagnostics view behind the existing Diagnostics owner; it cannot apply lifecycle events or change registry, route, mount, storage, or user data.',
    )
    ...
    print('PASS: Pass 5 Unit 4 runtime identities regenerated')
