...
def main() -> None:
    ...
    seal.update(
        ...
        sealed_branch="agent/pass10-unit3-bank-readonly-projection-v1",
        pass10_context=(
            "P10-U3 adds one Bank-owner read-only projection between the "
            "existing owner router and Bank tab. It preserves native identities "
            "and source bytes, quarantines unknown or conflicting facts, treats "
            "historic inventory as reference-only, exposes no raw payload or "
            "write/delete/migration API, and gives Helper registration and "
            "active-tab state no Bank authority."
        ),
    )
    SEAL.write_text(
    ...
    print("PASS: P10-U3 runtime integrity identities regenerated")
