...
def main() -> None:
    ...
    seal.update(
        ...
        sealed_branch="agent/pass10-unit7-bank-level-order-repair-v1",
        pass10_context=(
            "P10-U7R contains all Continuous Run owner and level nodes inside "
            "the single Bank detail slot, removes the self-triggering recurring "
            "Level 3-30B reparent loop, enforces exact stable order only when "
            "needed, and event-drives late external-node containment. The repair "
            "does not read, write, delete, migrate, or activate persisted data."
        ),
    )
    SEAL.write_text(
    ...
    print("PASS: P10-U7R Bank level-owner runtime integrity identities regenerated")
