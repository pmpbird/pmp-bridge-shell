...
REC=R/'audit/Packet_01.5_Pass_002_Dependency_Platform_Family_Independent_Verification_v1.json'

ALLOWED={
 '.github/workflows/packet_015_pass_002_dependency_platform_family.yml',
 'tools/build_packet_01_5_pass_002_dependency_platform_family_v1.py',
 'tools/verify_packet_01_5_pass_002_dependency_platform_family_v1.py',
...
def verify():
 ...
 for c,r,q in zip(census['records'],rem,source):
  ...
  assert r['family_inspection_status']=='NO_CURRENT_CLAIM_SPECIFIC_DEPENDENCY_OR_PLATFORM_RECEIPT'
  assert r['direct_decision_supported'] is False
  assert r['dependency_platform_receipts_reviewed']==0 and r['targeted_compatibility_tests_completed']==0
  assert r['effective_dependency_versions_verified'] is False
  assert r['effective_platform_state_verified'] is False and r['effective_provider_state_verified'] is False
 ...
 assert cov['direct_decision_gate_matches']==0 and cov['dependency_platform_receipts_reviewed']==0
 assert cov['targeted_compatibility_tests_completed']==0
 assert cov['effective_dependency_versions_verified'] is False
 assert cov['effective_platform_state_verified'] is False and cov['effective_provider_state_verified'] is False
 assert cov['pass_002_total_records']==122 and cov['previously_processed_records']==29
 assert cov['family_records_accounted']==4 and cov['processed_records_if_merged']==33
 assert cov['remaining_unprocessed_records']==89 and cov['pass_002_reconciliation_exact'] is True
 assert 29+4+89==122
 ...
 reject(lambda:require(cov['targeted_compatibility_tests_completed']==1)); n+=1
 reject(lambda:require(cov['effective_platform_state_verified'] is True)); n+=1
 reject(lambda:require(cov['dependencies_modified'] is True)); n+=1
 reject(lambda:require(cov['routing_assignments']==1)); n+=1
 reject(lambda:require(current_blob=='bad')); n+=1
 reject(lambda:require(deploy_blob=='bad')); n+=1
 reject(lambda:require('secret_value' not in FORBIDDEN_KEYS)); n+=1
 return {
  'packet':'01.5','verification':'pass_002_dependency_platform_family_independent','version':1,
  'status':'PASS_DEPENDENCY_PLATFORM_FAMILY_VERIFIED','authoritative_anchor':A,'family':F,
...
def main():
 ...
 print('STATUS: PASS — PASS 002 DEPENDENCY OR PLATFORM STATE FAMILY VERIFIED')
