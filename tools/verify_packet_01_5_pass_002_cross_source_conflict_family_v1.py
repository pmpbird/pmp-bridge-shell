...
REC=R/'audit/Packet_01.5_Pass_002_Cross_Source_Conflict_Family_Independent_Verification_v1.json'

ALLOWED={
 '.github/workflows/packet_015_pass_002_cross_source_conflict_family.yml',
 'tools/build_packet_01_5_pass_002_cross_source_conflict_family_v1.py',
 'tools/verify_packet_01_5_pass_002_cross_source_conflict_family_v1.py',
...
def verify():
 ...
 assert len(source)==4 and [x['composite_address'] for x in source]==ADDRESSES
 census,dec,rem,cov=fj(CENSUS),fl(DEC),fl(REM),fj(COV)
 assert census['family_records']==4 and census['addresses_in_inventory_order']==ADDRESSES and len(census['records'])==4
 assert dec==[] and len(rem)==4 and [x['composite_address'] for x in rem]==ADDRESSES
 for c,r,q in zip(census['records'],rem,source):
  ...
  assert r['family_inspection_status']=='NO_CURRENT_CLAIM_SPECIFIC_CONFLICT_ADJUDICATION_RECEIPT'
  assert r['direct_decision_supported'] is False
  assert r['conflict_adjudication_receipts_reviewed']==0 and r['conflicting_sources_identified']==0
  assert r['authority_statuses_verified'] is False and r['precedence_rules_verified'] is False
  assert r['current_authoritative_source_established'] is False
  ...
  assert 'authority' in r['smallest_exact_remaining_proof']
  assert 'precedence' in r['smallest_exact_remaining_proof']
 assert cov['family_records']==4 and cov['decided_records']==0 and cov['remaining_queued_records']==4
 assert cov['unknown_hold_created']==0 and cov['complete_nonduplicated_coverage']
 assert cov['direct_decision_gate_matches']==0 and cov['conflict_adjudication_receipts_reviewed']==0
 assert cov['conflicting_sources_identified']==0
 assert cov['authority_statuses_verified'] is False and cov['precedence_rules_verified'] is False
 assert cov['current_authoritative_source_established'] is False
 assert cov['pass_002_total_records']==122 and cov['previously_processed_records']==33
 assert cov['family_records_accounted']==4 and cov['processed_records_if_merged']==37
 assert cov['remaining_unprocessed_records']==85 and cov['pass_002_reconciliation_exact'] is True
 assert 33+4+85==122
 ...
 deploy_blob=git('rev-parse',f'{A}:{DEPLOY_RECEIPT}')
 dependency_blob=git('rev-parse',f'{A}:{DEPENDENCY_RECEIPT}')
 ...
 n=0
 reject(lambda:require(len(rem[:-1])==4)); n+=1
 ...
 reject(lambda:require(cov['family_records']==3)); n+=1
 reject(lambda:require(cov['conflict_adjudication_receipts_reviewed']==1)); n+=1
 reject(lambda:require(cov['conflicting_sources_identified']==1)); n+=1
 reject(lambda:require(cov['authority_statuses_verified'] is True)); n+=1
 reject(lambda:require(cov['precedence_rules_verified'] is True)); n+=1
 reject(lambda:require(cov['current_authoritative_source_established'] is True)); n+=1
 reject(lambda:require(cov['routing_assignments']==1)); n+=1
 reject(lambda:require(dependency_blob=='bad')); n+=1
 reject(lambda:require('secret_value' not in FORBIDDEN_KEYS)); n+=1
 return {
  'packet':'01.5','verification':'pass_002_cross_source_conflict_family_independent','version':1,
  'status':'PASS_CROSS_SOURCE_CONFLICT_FAMILY_VERIFIED','authoritative_anchor':A,'family':F,
  'family_records':4,'decisions_created':0,'remaining_exact_queues':4,'unknown_hold_created':0,
  ...
  'current_runtime_family_receipt_blob_sha':current_blob,
  'private_uncaptured_family_receipt_blob_sha':private_blob,
  'deployment_live_family_receipt_blob_sha':deploy_blob,
  'dependency_platform_family_receipt_blob_sha':dependency_blob,
...
def main():
 ...
 print('STATUS: PASS — PASS 002 CROSS-SOURCE CONFLICT FAMILY VERIFIED')
