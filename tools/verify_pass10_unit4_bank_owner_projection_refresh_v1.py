#!/usr/bin/env python3
from __future__ import annotations

import hashlib
import json
import re
import subprocess
import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
BASE = "1fe0f44486f1e30735cbb606762fc3c0a1652f2d"
REPORT = ROOT / "audit/pass10/pass10-bank-unit4-owner-projection-refresh-v1.json"
RECEIPT = (
    ROOT
    / "audit/pass10/receipts/"
    "RECEIPT_P10_U4_BANK_OWNER_PROJECTION_REFRESH_20260726T230500Z_001.json"
)
WORKFLOW = ROOT / ".github/workflows/pass10-unit4-bank-owner-projection-refresh-v1.yml"
UNIT3_REPORT = ROOT / "audit/pass10/pass10-bank-unit3-readonly-projection-v1.json"
BOUNDARY = ROOT / "pmp-bank-continuous-run-owner-boundary-v1.js"
PROJECTION = ROOT / "pmp-bank-inventory-readonly-projection-v1.js"
ADAPTER = ROOT / "pmp-bank-owner-projection-refresh-v1.js"
MASTER = ROOT / "pmp-master-bank-tab-v1.js"
INNER = ROOT / "pmp-current-inner-cleanbug-rgcontrols-v23.html"
MOUNTS = ROOT / "pmp-mount-registry-v1.js"
MANIFEST = ROOT / "pmp-runtime-integrity-manifest-v1.json"
SEAL = ROOT / "audit/a003-manifest-seal.json"
BOOTSTRAP = ROOT / "pmp-app-current.html"
GENERATOR = ROOT / "tools/generate_pass10_unit4_integrity_updates_v1.py"
TEST = ROOT / "tools/test_pass10_unit4_bank_owner_projection_refresh_v1.js"
UNIT3_TEST = ROOT / "tools/test_pass10_unit3_bank_readonly_projection_v1.js"
PASS9_TEST = (
    ROOT
    / "tools/test_pass9_unit3_bank_continuous_run_owner_integration_v1.js"
)
GATE = ROOT / "tools/run_pass6_unit7_no_blind_flying_gate_v1.py"
EXPECTED = {
    ".github/workflows/pass10-unit3-bank-readonly-projection-v1.yml",
    ".github/workflows/pass10-unit4-bank-owner-projection-refresh-v1.yml",
    "audit/a003-manifest-seal.json",
    "audit/pass10/pass10-bank-unit4-owner-projection-refresh-v1.json",
    (
        "audit/pass10/receipts/"
        "RECEIPT_P10_U4_BANK_OWNER_PROJECTION_REFRESH_20260726T230500Z_001.json"
    ),
    "pmp-app-current.html",
    "pmp-bank-owner-projection-refresh-v1.js",
    "pmp-current-inner-cleanbug-rgcontrols-v23.html",
    "pmp-master-bank-tab-v1.js",
    "pmp-mount-registry-v1.js",
    "pmp-runtime-integrity-manifest-v1.json",
    "tools/generate_pass10_unit4_integrity_updates_v1.py",
    "tools/test_pass10_unit4_bank_owner_projection_refresh_v1.js",
    "tools/verify_pass10_unit4_bank_owner_projection_refresh_v1.py",
}
IMPLEMENTATION = {
    "pmp-app-current.html",
    "pmp-bank-owner-projection-refresh-v1.js",
    "pmp-current-inner-cleanbug-rgcontrols-v23.html",
    "pmp-master-bank-tab-v1.js",
    "pmp-mount-registry-v1.js",
}
INPUTS = {
    "unit3_report_sha256": UNIT3_REPORT,
    "owner_refresh_adapter_sha256": ADAPTER,
    "readonly_projection_sha256": PROJECTION,
    "owner_boundary_sha256": BOUNDARY,
    "bank_tab_sha256": MASTER,
    "active_inner_sha256": INNER,
    "mount_registry_sha256": MOUNTS,
    "integrity_generator_sha256": GENERATOR,
    "test_sha256": TEST,
}


def output(*args: str) -> str:
    return subprocess.check_output(args, cwd=ROOT, text=True).strip()


def sha(path: Path) -> str:
    return hashlib.sha256(path.read_bytes()).hexdigest()


def changed_paths(base: str) -> set[str]:
    rows: set[str] = set()
    for command in (
        ("git", "diff", "--name-only", f"{base}...HEAD"),
        ("git", "diff", "--name-only", base),
        ("git", "ls-files", "--others", "--exclude-standard"),
    ):
        rows.update(filter(None, output(*command).splitlines()))
    return rows


def workflow_paths(text: str) -> set[str]:
    match = re.search(r"(?m)^    paths:\n(?P<rows>(?:      - [^\n]+\n)+)", text)
    assert match
    return {
        row.strip()[2:].strip().strip("'\"")
        for row in match.group("rows").splitlines()
    }


def assertion_total(command_output: str, label: str, expected: str) -> int:
    match = re.search(rf"{re.escape(label)} \((\d+)/(\d+)\)", command_output)
    assert match and match.group(1) == match.group(2) == expected, command_output
    return int(match.group(1))


def main() -> None:
    base = sys.argv[1] if len(sys.argv) > 1 and sys.argv[1] else BASE
    assert base == BASE
    actual = changed_paths(base)
    assert actual == EXPECTED, (sorted(actual), sorted(EXPECTED))

    report = json.loads(REPORT.read_text())
    receipt = json.loads(RECEIPT.read_text())
    unit3 = json.loads(UNIT3_REPORT.read_text())
    assert report["base_main_commit"] == BASE
    assert report["unit_id"] == "P10-U4"
    assert report["status"] == "BANK_OWNER_PROJECTION_REFRESH_PROVEN"
    assert set(report["scope"]["changed_paths"]) == EXPECTED
    assert set(report["scope"]["implementation_paths"]) == IMPLEMENTATION
    assert set(receipt["changed_paths"]) == EXPECTED
    assert receipt["status"] == report["status"]
    assert unit3["status"] == "BANK_READONLY_PROJECTION_PROVEN"
    assert unit3["next_step"]["id"] == "P10-U4"
    for key, path in INPUTS.items():
        assert report["inputs"][key] == sha(path), (
            key,
            report["inputs"][key],
            sha(path),
        )

    assertions = assertion_total(
        output("node", str(TEST.relative_to(ROOT))),
        "refresh",
        "121",
    )
    assertion_total(
        output("node", str(UNIT3_TEST.relative_to(ROOT))),
        "projection",
        "125",
    )
    assertion_total(
        output("node", str(PASS9_TEST.relative_to(ROOT))),
        "owner integration",
        "234",
    )
    matrix = report["deterministic_matrix"]
    assert matrix["assertions"] == assertions
    assert matrix["assertions_passed"] == assertions
    assert matrix["assertions_failed"] == 0
    assert receipt["coverage"]["unit4_assertions"] == assertions
    assert receipt["coverage"]["unit3_regression_assertions"] == 125
    assert receipt["coverage"]["pass9_owner_regression_assertions"] == 234

    integration = report["integration"]
    assert integration["model"] == (
        "EXACT_OWNER_RECEIPT_ONE_PROJECTION_REFRESH_SANITIZED_UI_EVENT"
    )
    assert integration["owner"] == "bank_screen_owner"
    assert integration["owner_event"] == "pmp:bank-owner-write-committed"
    assert integration["sanitized_projection_event"] == (
        "pmp:bank-inventory-projection-refreshed"
    )
    assert integration["accepted_event_projection_refreshes"] == 1
    for key in (
        "automatic_retry_limit",
        "recurring_timers",
        "adapter_storage_reads",
        "adapter_storage_writes",
        "adapter_storage_deletes",
        "raw_payloads_exposed",
        "write_apis_exposed",
        "delete_apis_exposed",
        "migration_apis_exposed",
    ):
        assert integration[key] == 0, key
    for key in (
        "helper_registration_conveys_authority",
        "active_tab_conveys_authority",
        "direct_ui_event_conveys_authority",
    ):
        assert integration[key] is False, key
    assert integration["single_adapter_load"] is True
    assert integration["single_adapter_mount"] is True
    assert integration["bank_tab_raw_owner_listener_removed"] is True
    assert integration["bank_tab_cached_projection_render"] is True
    assert integration["explicit_user_refresh_preserved"] is True
    assert len(integration["receipt_validation"]) == 8
    assert len(integration["fail_closed_cases"]) == 10

    adapter = ADAPTER.read_text()
    master = MASTER.read_text()
    projection = PROJECTION.read_text()
    inner = INNER.read_text()
    mounts = MOUNTS.read_text()
    for token in (
        "PMP_BANK_CONTINUOUS_RUN_OWNER_RECEIPT_V1",
        "receipt.requester_owner!==OWNER",
        "processed.has(hash)",
        "boundary.resourceVersion(resource)!==version",
        "receiptHashValid(boundary,receipt)",
        "recentReceiptValid(boundary,receipt)",
        "projection.snapshot('accepted_bank_owner_commit:'",
        "direct_ui_event_conveys_authority:false",
        "window.addEventListener('pmp:bank-owner-write-committed',onOwnerCommit)",
    ):
        assert token in adapter, token
    for forbidden in (
        "setInterval(",
        "setTimeout(",
        ".setItem(",
        ".removeItem(",
        "indexedDB.open",
    ):
        assert forbidden not in adapter, forbidden
    assert (
        "window.addEventListener("
        "'pmp:bank-owner-write-committed',scan)"
    ) not in master
    for token in (
        (
            "window.addEventListener("
            "'pmp:bank-inventory-projection-refreshed',ownerProjectionRefreshed)"
        ),
        "state.last_code!=='REFRESHED_EXACTLY_ONCE'",
        "hash===lastOwnerRefreshReceipt",
        "p.lastSnapshot?p.lastSnapshot():null",
        "p.snapshot('bank_tab_initial_projection')",
        "paint(d,true,'bank_tab_user_refresh')",
        "Helper registration and active tab convey no authority",
    ):
        assert token in master, token
    for forbidden in (
        ".recordWrite(",
        ".recordDelete(",
        "localStorage.setItem",
        "setInterval(",
    ):
        assert forbidden not in master, forbidden
    assert ".setItem(" not in projection
    assert ".removeItem(" not in projection

    boundary_index = inner.index("pmp-bank-continuous-run-owner-boundary-v1.js")
    router_index = inner.index("pmp-master-bank-inventory-router-v1.js")
    projection_index = inner.index("pmp-bank-inventory-readonly-projection-v1.js")
    adapter_index = inner.index("pmp-bank-owner-projection-refresh-v1.js")
    tab_index = inner.index("pmp-master-bank-tab-v1.js")
    assert (
        0
        <= boundary_index
        < router_index
        < projection_index
        < adapter_index
        < tab_index
    )
    assert inner.count("pmp-bank-owner-projection-refresh-v1.js") == 1
    assert mounts.count("pmp-bank-owner-projection-refresh-v1.js") == 1

    before = {
        path: sha(path)
        for path in (MANIFEST, SEAL, BOOTSTRAP)
    }
    subprocess.check_call(
        ["python3", str(GENERATOR.relative_to(ROOT))],
        cwd=ROOT,
    )
    after = {
        path: sha(path)
        for path in (MANIFEST, SEAL, BOOTSTRAP)
    }
    assert before == after, (before, after)
    manifest = json.loads(MANIFEST.read_text())
    seal = json.loads(SEAL.read_text())
    records = {row["path"]: row for row in manifest["records"]}
    assert manifest["counts"]["runtime_records"] == 712
    assert manifest["counts"]["executable_records"] == 712
    assert len(manifest["records"]) == 712
    for path in (ADAPTER, MASTER, INNER, MOUNTS):
        relative = path.relative_to(ROOT).as_posix()
        assert records[relative]["sha256_hex"] == sha(path), relative
    assert report["runtime_integrity"] == {
        "runtime_records": 712,
        "manifest_sha256": sha(MANIFEST),
        "runtime_source_set_sha256": manifest["runtime_source_set_sha256"],
        "seal_sha256": sha(SEAL),
        "bootstrap_sha256": sha(BOOTSTRAP),
    }
    assert seal["status"] == "SEALED"
    assert seal["sealed_branch"] == report["branch"]
    assert seal["manifest_sha256"] == sha(MANIFEST)
    assert seal["runtime_source_set_sha256"] == (
        manifest["runtime_source_set_sha256"]
    )
    assert (
        f"const MANIFEST_SHA256='{sha(MANIFEST)}';"
        in BOOTSTRAP.read_text()
    )
    for token in (
        "one bounded owner-refresh adapter",
        "exact accepted Bank-owner receipt",
        "Helper, active-tab, and direct UI events fail closed",
    ):
        assert token in seal["pass10_context"], token

    binding = report["no_blind_flying_gate"]
    assert binding["ci_lane"] == "deterministic_integration"
    assert binding["diagnostic_matrix_update"]["status"] == "ADDED"
    assert binding["fault_injection"]["status"] == "COVERED"
    assert len(binding["fault_injection"]["cases"]) == 10
    assert len(binding["observed_facts"]) == 2
    assert len(binding["inferred_conclusions"]) == 1
    assert len(binding["required_artifact_roles"]) == 9
    assert binding["upload_before_enforcement"] is True
    assert binding["automatic_retry"] is False
    assert binding["special_authority"] == {
        "required": False,
        "granted": False,
        "consumed": False,
    }
    gate = json.loads(
        output("python3", str(GATE.relative_to(ROOT)), "--base", BASE)
    )
    assert gate["status"] == "PASS", gate
    assert gate["unit_id"] == "P10-U4"
    assert gate["summary"]["runtime_paths"] == 5
    assert gate["errors"] == []

    workflow = WORKFLOW.read_text()
    assert workflow_paths(workflow) == EXPECTED
    for token in (
        "actions/setup-node@v4",
        "node-version: '22'",
        "actions/setup-python@v5",
        "python-version: '3.12'",
        "if: always()",
        "actions/upload-artifact@v4",
        "Upload complete P10-U4 evidence",
        "Enforce preserved result after upload",
        "artifact-manifest.json",
        "authority-state.json",
        "exit-status.json",
        "scope.json",
        "retention-days: 90",
    ):
        assert token in workflow
    assert workflow.index("Upload complete P10-U4 evidence") < workflow.index(
        "Enforce preserved result after upload"
    )
    assert workflow.rstrip().endswith(
        'run: test "${{ steps.evaluate.outputs.exit_code }}" = "0"'
    )

    assert report["effects"]["persisted_user_data_changed"] is False
    assert report["effects"]["storage_migration_performed"] is False
    assert report["effects"]["live_observation_performed"] is False
    assert report["effects"]["formal_proof_performed"] is False
    assert receipt["effects"]["persisted_user_data_changed"] is False
    assert receipt["effects"]["storage_migration_performed"] is False
    assert report["authority"]["special_authority_consumed"] is False
    assert report["next_step"]["id"] == "P10-U5"
    assert report["next_step"]["requires_user_app_check"] is False
    assert report["next_step"]["persisted_user_data_change_allowed"] is False
    assert receipt["next_safe_move"]["step_id"] == "P10-U5"
    assert receipt["next_safe_move"]["requires_user_app_check"] is False
    print(
        "PASS: exact fourteen-file P10-U4 Bank owner projection refresh "
        "verified (121/121, P10-U3 125/125, P9-U3 234/234, "
        "gate PASS, integrity resealed, P10-U5 ready)"
    )


if __name__ == "__main__":
    main()
