(()=>{
'use strict';
const V='1.0.1-fixed-bottom-control-hash-only';
...
function controlHashOnly(w){try{return String((w&&w.location&&w.location.hash)||'').toLowerCase()==='#control'}catch(e){return false}}
...
function install(){eachDoc((d,w)=>{try{removeAll(d);if(!controlHashOnly(w))return;let st=status(),ok=st.pass7_current;let foot=d.createElement('div');foot.id='pmpCurrentPathControlFooterV1';foot.setAttribute('data-pmp-section-owner','current_path_control_footer');foot.setAttribute('data-pmp-fixed-control-only','1');foot.textContent=(ok?'PASS 7 CURRENT':'OLD ROUTE DETECTED')+' · v28 '+st.seen.v28+' · v29 '+st.seen.v29+' · old '+st.old_route_files.length;foot.style.cssText='position:fixed;left:10px;right:10px;bottom:calc(74px + env(safe-area-inset-bottom));z-index:2147483000;pointer-events:none;box-sizing:border-box;padding:10px 12px;text-align:center;border:3px solid #07101c;border-radius:999px;background:'+(ok?'#dfffe4':'#ffe1df')+';color:#07101c;font:950 13px -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;box-shadow:0 8px 24px rgba(0,0,0,.20);white-space:nowrap;overflow:hidden;text-overflow:ellipsis';d.body.appendChild(foot);try{localStorage.setItem(RECEIPT_KEY,JSON.stringify(st,null,2))}catch(e){}}catch(e){}})}
