(function(){
  ...
  function copyText(d,val){try{navigator.clipboard.writeText(val)}catch(e){const ta=d.createElement('textarea');ta.value=val;d.body.appendChild(ta);ta.select();d.execCommand('copy');ta.remove()}}
  function makeBigButton(d,id,title,sub,icon,onClick){
    const btn=d.createElement('button');
    btn.id=id; btn.className='big'; btn.type='button';
    btn.style.width='100%'; btn.style.marginTop='10px'; btn.style.background='var(--a,#acd1fb)'; btn.style.color='var(--buttonText,#07101c)'; btn.style.border='var(--buttonBorder,2px) solid var(--line,#07101c)';
    btn.innerHTML='<span class="icon">'+esc(icon||'▣')+'</span><span>'+esc(title)+'<small>'+esc(sub||'')+'</small></span><span class="chev">›</span>';
  ...
  function addConnectionButton(d){
    ...
    const btn=makeBigButton(d,'pmpOpenBankInventoryBtn','Bank Inventory','protected registry / do-not-overwrite map','▣',()=>renderPanel(d));
    p.appendChild(btn);
  }
  function removeOldBridgeButton(d){
    try{const old=d&&d.getElementById&&d.getElementById('pmpBridgeBankInventoryCardBtn'); if(old)old.remove()}catch(e){}
  }
  function patch(){
    const w=panelWin(), d=panelDoc(); if(!w||!d)return;
    removeOldBridgeButton(d);
    ...
    try{addConnectionButton(d)}catch(e){}
  }
