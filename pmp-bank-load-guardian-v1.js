(()=>{
'use strict';
const V='1.0.1-active-bank-only-detection-20260710A';
...
function repairIfBlank(reason){let r=evaluate(reason||'repair_check');if(!r.bank_shell_visible||r.bank_inventory_visible){lastBlankAt=0;put(KEY,{type:'PMP_BANK_LOAD_GUARDIAN_V1_RECEIPT',version:V,owner:OWNER,at:now(),status:r.bank_shell_visible?'no_blank_bank_repair_needed':'no_active_bank_screen_detected',bank_shell_visible:r.bank_shell_visible,bank_inventory_visible:r.bank_inventory_visible,blank_bank_shell_detected:r.blank_bank_shell_detected,side_effects:r.side_effects});return r}
 let age=lastBlankAt?Date.now()-lastBlankAt:0;if(!lastBlankAt){lastBlankAt=Date.now();put(KEY,{type:'PMP_BANK_LOAD_GUARDIAN_V1_RECEIPT',version:V,owner:OWNER,at:now(),status:'blank_bank_detected_waiting_for_native_inventory',bank_shell_visible:true,bank_inventory_visible:false,blank_bank_shell_detected:true,wait_ms_before_guardian:1000,side_effects:r.side_effects});return r}
 if(age<1000){put(KEY,{type:'PMP_BANK_LOAD_GUARDIAN_V1_RECEIPT',version:V,owner:OWNER,at:now(),status:'blank_bank_waiting',elapsed_ms:age,bank_shell_visible:true,bank_inventory_visible:false,blank_bank_shell_detected:true,side_effects:r.side_effects});return r}
