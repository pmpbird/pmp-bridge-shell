(()=>{
'use strict';
const V='1.0.2-includes-home-visibility-receipt-20260710A';
...
function relatedHome(){return {home_visibility_guard_receipt:read(HOME_RECEIPT)||{status:'not_run_or_no_receipt_yet',key:HOME_RECEIPT},home_visibility_guard_report:read(HOME_REPORT)||{status:'not_run_or_no_report_yet',key:HOME_REPORT}}}
...
function receipt(status,extra,side){let r=Object.assign({type:'PMP_BANK_LOAD_GUARDIAN_V1_RECEIPT',version:V,owner:OWNER,at:now(),status,related_receipts:relatedHome(),side_effects:side||baseSideEffects()},extra||{});put(KEY,r);return r}
function repairIfBlank(reason){let r=evaluate(reason||'repair_check');if(!r.bank_shell_visible||r.bank_inventory_visible){lastBlankAt=0;receipt(r.bank_shell_visible?'no_blank_bank_repair_needed':'no_active_bank_screen_detected',{bank_shell_visible:r.bank_shell_visible,bank_inventory_visible:r.bank_inventory_visible,blank_bank_shell_detected:r.blank_bank_shell_detected},r.side_effects);return r}
 ...
 try{let wrap=c.d.createElement('div');wrap.innerHTML=cardHTML();let inv=wrap.firstElementChild;let mount=sf.mount||c.el;mount.appendChild(inv);lastBlankAt=0;let out=evaluate('guardian_restored_visible_inventory');out.action='restored_visible_inventory_cards_only';out.guardian_note='Restored missing visible Bank inventory card surface after native inventory stayed blank. Did not modify Bank records, IndexedDB, or storage.';out.related_receipts=relatedHome();put(REPORT,out);receipt('blank_bank_shell_repaired_visible_inventory_only',{action:out.action,bank_shell_visible:true,bank_inventory_visible:true,blank_bank_shell_detected:false},out.side_effects);return out}catch(e){receipt('repair_failed',{error:String(e&&e.message||e)},r.side_effects);return r}}
function run(reason){return repairIfBlank(reason||'run')}
window.PMPBankLoadGuardianV1={version:V,owner:OWNER,run,evaluate,rule:'Active Bank screen only. Detects Bank by actual Bank title plus Bank subtitle, excluding Diagnostics and nav. Report includes Bank Home Visibility Guard receipt/report. Does not change records, IndexedDB, storage, or routes.'};try{T().PMPBankLoadGuardianV1=window.PMPBankLoadGuardianV1}catch(e){};
