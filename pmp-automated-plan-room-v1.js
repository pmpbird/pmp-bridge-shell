...
function renderLoading(doc){buildOverlay(doc,function(wrap){let card=doc.createElement('section');card.className='pmp-ap-card card';let title=doc.createElement('h1');title.textContent=MAIN_LABEL;let sub=doc.createElement('p');sub.className='sub';sub.textContent='Loading plan state…';let back=doc.createElement('button');back.className='big';back.innerHTML='<span class="icon">←</span><span>Back to Control Room<small>return without changing plan state</small></span><span class="chev">›</span>';back.onclick=function(e){if(e)e.preventDefault();return closeRoom(doc)};card.append(title,sub,back);wrap.appendChild(card)})}
...
async function openRoom(doc){if(cachedState){renderRoom(doc);return false}renderLoading(doc);try{await refreshData();if(doc.getElementById('pmpAutomatedPlanOverlayV1'))renderRoom(doc)}catch(e){let n=doc.getElementById('pmpAutomatedPlanRoomNoteV1');if(n)n.textContent='Plan state could not be loaded. No state was changed. '+String(e&&e.message||e);else renderRoom(doc)}return false}
