...
let cachedState=null,cachedPolicy=null;
...
async function readJson(path){let r=await fetch(path+'?fresh='+Date.now(),{cache:'no-store'});if(!r.ok)throw new Error(path+' status '+r.status);return r.json()}
async function refreshData(){let values=await Promise.all([readJson(STATE_URL),readJson(POLICY_URL)]);cachedState=values[0];cachedPolicy=values[1];return values}
function genericStatus(state){let s=String(state&&state.status||'idle').toLowerCase();if(s==='running')return'Running';if(s==='paused')return'Paused';if(s==='stopped')return'Stopped';if(s==='complete')return'Complete';if(s==='setup')return'Setup';return'Idle'}
function statusLine(state){let s=genericStatus(state);if(s==='Setup')return'Setup — execution is safely locked';if(s==='Paused')return'Paused — ready to continue';if(s==='Running')return'Running — verified work in progress';if(s==='Stopped')return'Stopped — details contain the exact reason';if(s==='Complete')return'Complete — final receipt preserved';return'No active automated work'}
