...
#pmpAutomatedPlanOverlayV1 .pmp-ap-card{background:var(--card,#fff);border:var(--borderWidth,2px) solid var(--line,#07101c);border-radius:28px;padding:18px;margin:0 0 16px;box-shadow:var(--cardShadow,var(--shadow,0 12px 28px #0002));contain:layout paint}
#pmpAutomatedPlanOverlayV1 .pmp-ap-grid{display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important;margin:10px 0 0!important;contain:layout}
#pmpAutomatedPlanOverlayV1 .pmp-ap-grid>.mini{width:100%!important;margin:0!important;min-width:0!important}
...
@media(max-width:520px){#pmpAutomatedPlanOverlayV1 .pmp-ap-kv{grid-template-columns:1fr}}
...
function buildOverlay(doc,bodyBuilder){css(doc);let old=doc.getElementById('pmpAutomatedPlanOverlayV1');if(old)old.remove();let overlay=doc.createElement('div');overlay.id='pmpAutomatedPlanOverlayV1';let wrap=doc.createElement('div');wrap.className='pmp-ap-wrap';overlay.appendChild(wrap);doc.body.appendChild(overlay);bodyBuilder(wrap);return overlay}
function renderRoom(doc){buildOverlay(doc,function(wrap){let card=doc.createElement('section');card.className='pmp-ap-card card';let title=doc.createElement('h1');title.textContent=MAIN_LABEL;let sub=doc.createElement('p');sub.className='sub';sub.id='pmpAutomatedPlanRoomStatusV1';sub.textContent=controllerLine(cachedState,cachedController,cachedRuntime);let back=doc.createElement('button');back.className='big';back.innerHTML='<span class="icon">←</span><span>Back to Control Room<small>return without changing plan state</small></span><span class="chev">›</span>';back.onclick=function(e){if(e)e.preventDefault();return closeRoom(doc)};let controls=doc.createElement('div');controls.className='pmp-ap-grid';controls.style.cssText='display:grid!important;grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:8px!important;margin:10px 0 0!important;contain:layout';let refresh=doc.createElement('button');refresh.className='mini';refresh.style.cssText='width:100%!important;margin:0!important;min-width:0!important';refresh.textContent='Refresh';let detailsButton=doc.createElement('button');detailsButton.className='mini';detailsButton.style.cssText='width:100%!important;margin:0!important;min-width:0!important';detailsButton.textContent='Details';let note=doc.createElement('div');note.className='note';note.id='pmpAutomatedPlanRoomNoteV1';note.textContent=cachedController&&cachedController.safe_message||'The plan identity and checkpoint are stored internally. The main Control Room button stays universal.';let details=doc.createElement('div');details.className='panel pmp-ap-details hidden';details.id='pmpAutomatedPlanDetailsV1';let values=detailValue(cachedState,cachedPolicy,cachedController,cachedRuntime);Object.keys(values).forEach(k=>details.appendChild(kv(doc,k,values[k])));detailsButton.onclick=function(){details.classList.toggle('hidden');return false};refresh.onclick=async function(){refresh.disabled=true;try{await refreshData();renderRoom(doc)}catch(e){note.textContent='Unable to refresh plan state. No state was changed. '+String(e&&e.message||e)}finally{refresh.disabled=false}};controls.append(refresh,detailsButton);card.append(title,sub,back,controls,note,details);wrap.appendChild(card)});return false}
async function openRoom(doc){if(!cachedState)await refreshData().catch(function(){});renderRoom(doc);return false}
