...
const installed=[];
function installWin(w){try{if(!w||installed.indexOf(w)>-1)return;installed.push(w);w.addEventListener('error',e=>{try{let target=e.target||{};let tag=String(target.tagName||'').toLowerCase();if(tag==='script'||tag==='link'||tag==='iframe'||tag==='frame'){capture('Resource Load Failure','Active app resource load',tag+' failed to load.',{tag,src:target.src||target.href||target.getAttribute&&target.getAttribute('src')||'',document_url:String(w.location&&w.location.href||'')},'high');return}if(e.message)capture('Runtime Error','Active app runtime',String(e.message),{filename:e.filename,lineno:e.lineno,colno:e.colno,document_url:String(w.location&&w.location.href||'')},'high')}catch(_){}} ,true);w.addEventListener('unhandledrejection',e=>{try{capture('Unhandled Promise Rejection','Active app runtime',String(e.reason&&e.reason.message||e.reason||'Unhandled promise rejection'),{document_url:String(w.location&&w.location.href||'')},'high')}catch(_){}})}catch(e){}}
