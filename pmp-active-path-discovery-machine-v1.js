...
function lane(path,from){let p=String(path||'').toLowerCase(),f=String(from||'').toLowerCase();if(/test|candidate|repo-index|route-health|path-certifier|simulation|diagnostic|archive|history|old-panel|last-known|legacy/.test(p))return'legacy_or_test_candidate';if(/last-good|recovery|restore|safe-writer|code-safety|safety|resident|clean-v21|clean-v22/.test(p)||/last-good|recovery/.test(f))return'fallback_or_recovery_candidate';if(CURRENT_BOOT_ROOT.includes(path))return'current_boot_root';return'direct_current_candidate'}
...
async function copyNow(d,r,out){r=r||LAST||await run();let txt=compactProof(r);await copyText(d,txt);try{localStorage.setItem(COPY_RECEIPT_KEY,JSON.stringify({type:'PMP_ACTIVE_PATH_DISCOVERY_COPY_RECEIPT_V1',version:VERSION,at:now(),mode:'compact_freeze_proof_no_download',hard_missing_count:r.hard_missing_count,dead_reference_count:r.dead_reference_count,freeze_gate_pass:r.freeze_gate&&r.freeze_gate.pass,text_length:txt.length},null,2))}catch(e){}if(out)out.textContent='Small Discovery Freeze Proof copied. Paste it into ChatGPT. '+summary(r)+' · freeze_gate_pass: '+(r.freeze_gate&&r.freeze_gate.pass)}
