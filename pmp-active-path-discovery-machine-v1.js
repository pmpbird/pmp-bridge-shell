(()=>{
'use strict';
const VERSION='1.1.0-v30-current-chain-atlas-support-20260709A';
...
const IGNORE_REFERENCES=['discovery-report.json','freeze-proof.txt','blocker-detail.txt','PDF.js'];
...
function ignoredPath(x){return IGNORE_REFERENCES.some(p=>String(x||'')===p||String(x||'').toLowerCase()===p.toLowerCase())}
function cleanPath(x){x=String(x||'').replace(/&amp;/g,'&').trim().split('#')[0].split('?')[0].replace(/^\.\//,'').replace(/^\//,'');if(!x||ignoredPath(x))return'';if(/^https?:/i.test(x)||x.includes('://')||x.includes('..'))return'';let m=x.match(/^[a-zA-Z0-9._\/-]+\.(?:html|js|json)$/i);return m?x:''}
