(()=>{
  ...
  async function scan(){let c=ctx(),map=await loadMap(),m=mapped(map,c),cmp=compare(m.files,c.scripts),out={type:'PMP_CURRENT_PAGE_CODE_SCOPE_V1_RECEIPT',version:V,at:new Date().toISOString(),mode:'quiet_current_page_only',scope:'current_page_only_not_whole_app',context:c,map_loaded:!!map,map_version:map&&map.version||'',matched_map_source:m.source,scope_kind:m.kind,scope_key:m.key,repo_files:m.files,repo_files_count:m.files.length,live_files:c.scripts,live_files_count:c.scripts.length,comparison:cmp,pass:cmp.pass,status:cmp.status,flags:cmp.flags,note:'Only the current page is checked. If a bank detail is open, only that bank detail map is used; otherwise only the current route map is used.'};write(OUT,out);let r=read(RELOAD,null);if(r&&typeof r==='object'){r.current_page_code_scope=out;r.current_page_code_scope_key=OUT;write(RELOAD,r)}return out}
