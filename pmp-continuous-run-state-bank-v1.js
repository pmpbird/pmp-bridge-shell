(function(){
  ...
  function defaults(){return{
    type:'PMP_PERSISTENT_CONTINUOUS_RUN_STATE_BANK_V1',version:VERSION,owner:OWNER,created_at:now(),updated_at:now(),manual_clear_required:true,
    current_packet:'',current_item:'',packet_queue:[],packet_1_5_active_items:[],last_valid_receipt:null,next_safe_action:'',
    blocked_items:[],watch_items:[],passed_items:[],helper_owner_registry:{},context_recovery_state:{status:'not_checked',last_check_at:null,missing:[]},
    last_run_status:'not_started',last_stop_reason:'',last_resume_point:'',export_snapshot:null,notes:''
  }}
  ...
  function buildManifest(state,receipts){const stateText=JSON.stringify(state||readRunState());const receiptText=JSON.stringify(receipts||readReceipts());return{type:'PMP_CONTINUOUS_RUN_STATE_MANIFEST_V1',owner:OWNER,at:now(),state_key:STATE_KEY,receipts_key:RECEIPTS_KEY,manifest_key:MANIFEST_KEY,state_chars:stateText.length,receipt_count:(receipts||readReceipts()).length,state_hash:hash(stateText),receipts_hash:hash(receiptText),manual_clear_required:true}}
  ...
  function recordStop(reason){return writeRunState({last_run_status:'stopped',last_stop_reason:String(reason||'manual_or_unknown'),last_resume_point:(readRunState().current_packet||'')+' / '+(readRunState().current_item||'')},{type:'PMP_RUN_STOP_RECORDED',reason:String(reason||'manual_or_unknown')})}
