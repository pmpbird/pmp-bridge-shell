(() => {
  ...
  function drawerState(d) {
    ...
    for (const el of candidates) {
      ...
      if (t.includes('launcher') && t.includes('reload') && t.includes('last good')) launcher = true;
    }
  ...
  function topButtonVisible() { try { const b = window.top.document.getElementById('pmpTopCopyLosslessButton'); return !!b && getComputedStyle(b).display !== 'none'; } catch (_) { return false; } }
  function buildReport(trigger) {
    ...
    const residentButtonReady = drawers.resident && topButtonVisible();
    const checks = [
      ...
      { name: 'Resident-open final tap allowed', pass: !drawers.launcher && (!drawers.resident || residentButtonReady), detail: drawers.launcher ? 'Launcher blocks final tap' : drawers.resident ? (residentButtonReady ? 'Resident open; Resident-visible Copy Lossless button is ready' : 'Resident open but Resident-visible Copy Lossless button is not visible yet') : 'No drawer blocking final tap' }
    ];
    ...
    return { type: 'PMP_RESIDENT_LOSSLESS_READINESS_CHECK', version: '1.2.0-resident-open', built_at: new Date().toISOString(), trigger: trigger || 'resident', status, summary, resident_open: drawers.resident, launcher_open: drawers.launcher, core_ready: checks.slice(0, 9).every(c => c.pass), final_tap_available: ready, checks, manual_final_step: 'Tap Copy Lossless Report as a real top-shell tap. This can be done from Bridge or from the Resident-visible Copy Lossless button.', after_manual_step: 'After the Shortcut finishes, ask ChatGPT to check the vault current record.', guardrail: 'Resident Copy Work copies diagnostic work only and can overwrite the clipboard, so do not use Copy Work during the vault-write test.' };
  }
  function showReport(report) { const o = deep(), d = o.d; if (!d) return; const reply = d.getElementById('residentReply'), work = d.getElementById('residentWork'); if (reply) reply.textContent = report.summary; if (work) { work.classList.remove('hidden'); work.textContent = JSON.stringify(report, null, 2); } try { localStorage.setItem('pmp_resident_lossless_readiness_latest_v1', JSON.stringify(report)); } catch (_) {} }
