(()=>{
'use strict';
const V='1.0.0-resident-continuous-run-status-reader';
...
function transferFallback(){
  ...
  return {
    ok:!!(m&&(m.verified||m.lossless_verified)),
    reason:m?(m.lossless_verified?'lossless_verified':'temporary_transfer_store_not_lossless_verified'):'temporary_transfer_store_manifest_missing',
    missing:missing.length?missing:(m?[]:['transfer_store_manifest']),
    slot_check_passed:!!(m&&(m.slot_check_passed||(v.slot_check_passed))),
    lossless_verified:!!(m&&(m.lossless_verified||(v.lossless_verified))),
    weak_items:arr(v.weak_items),
    manifest:m
  };
}
function transferGate(){
  try{
    const api=window.PMPContinuousRunBankTransferStoreV1;
    if(api&&typeof api.engineGate==='function')return api.engineGate();
  }catch(e){return {ok:false,reason:'transfer_store_api_error',missing:['transfer_store_api_error'],message:e.message,weak_items:[]}}
  return transferFallback();
}
function unitProof(){
  const u=units(),src=sourceText();
  const watch=[];
...
function buildStatus(w,d){
  const g=transferGate();
  const up=unitProof();
  const pg=proof();
  const r=runner();
  const rp=json(K.runnerProof,null);
  const manifest=g.manifest||json(K.transferManifest,null);
  const state=json(K.engineState,null);
  const receipts=arr(json(K.engineReceipts,[]));
  ...
  const transferMissing=arr(g.missing);
  const blocked=arr(pg&&pg.blocked_units).length?arr(pg.blocked_units):up.blocked_units;
  const out={
    type:'PMP_RESIDENT_CONTINUOUS_RUN_STATUS_READER_V1',
    version:V,
    owner:OWNER,
    at:now(),
    read_only:true,
    resident_access:'direct_read_from_resident_context',
    bank_access:{
      can_read_continuous_run_bank:true,
      transfer_store_verified:!!g.ok,
      reason:g.reason||'',
      slot_check_passed:!!g.slot_check_passed,
      lossless_verified:!!g.lossless_verified,
      missing:transferMissing,
      weak_items_count:arr(g.weak_items).length,
      manifest_present:!!manifest,
      receipt_count:arr(json(K.transferReceipts,[])).length
    },
    engine_access:{
      can_read_engine_state:true,
      runner_status:runnerStatus,
      proof_gate_status:pgStatus,
      proof_gate_ready:pgStatus==='PASS'||!!(pg&&pg.work_ready),
      source_characters:(pg&&Number(pg.source_characters))||up.source_characters,
      units_total:(pg&&Number(pg.units_total))||up.units_total,
      units_proven:(pg&&Number(pg.units_proven))||0,
      watch_units:(pg&&Number(pg.watch_units))||up.watch_units,
      blocked_units:blocked,
      completed:Number(r&&r.units_completed||0),
      total:Number(r&&r.total_units||up.units_total||0),
      engine_state_present:!!state,
      engine_receipt_count:receipts.length
    },
    dom_status:domStatus(w,d),
    plain:'',
    next:''
  };
  const bankLine='Continuous Run Bank transfer store: '+(out.bank_access.transfer_store_verified?'VERIFIED':'NOT VERIFIED');
  const missingLine='Source/transfer store missing: '+(out.bank_access.missing.length?out.bank_access.missing.join(', '):'none');
  let why='';
  ...
  out.plain=[
    'I can directly read the Continuous Run Bank and Continuous Work Engine status.',
    bankLine+'.',
    missingLine+'.',
    'Runner status: '+out.engine_access.runner_status+'.',
    'Proof Gate: '+out.engine_access.proof_gate_status+'.',
    'Work source characters: '+out.engine_access.source_characters+'. Work units: '+out.engine_access.units_total+'.',
    'Why blocked: '+why
  ].join('\n');
...
function showResident(w,d,out){
  try{save(K.lastReader,out)}catch(e){}
...
function installInto(w,d){
  if(!w||!d||w.__pmpResidentContinuousRunStatusReaderV1)return;
  w.__pmpResidentContinuousRunStatusReaderV1=true;
  w.PMPResidentContinuousRunStatusReaderV1={version:V,read:()=>buildStatus(w,d),copy:()=>{const out=buildStatus(w,d),txt=JSON.stringify(out,null,2);try{navigator.clipboard.writeText(txt)}catch(e){}return out}};
  const old=typeof w.residentRun==='function'?w.residentRun:null;
  w.residentRun=function(){
    const q=(d.getElementById('ask')||{}).value||'';
    if(wantsStatus(q)){
      const out=buildStatus(w,d);
      showResident(w,d,out);
      return false;
    }
