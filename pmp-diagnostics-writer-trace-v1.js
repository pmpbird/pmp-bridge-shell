...
function liveReceipt(){return completeReceipt(LIVE_BCD_RECEIPT)?LIVE_BCD_RECEIPT:read(BCD_RECEIPT_KEY)}
...
function publishBounded(produced){const published=boundedReceipt(produced),bytes=JSON.stringify(published).length;LIVE_BCD_RECEIPT=published;try{T().localStorage.setItem(BCD_RECEIPT_KEY,JSON.stringify(published));const observed=read(BCD_RECEIPT_KEY);return{ok:completeReceipt(observed),receipt:observed,bytes,mode:'localStorage_bounded',persistence_status:'PERSISTED'}}catch(error){const virtualized=installVirtualReceiptRead(),observed=liveReceipt();return{ok:completeReceipt(observed),receipt:observed,bytes,mode:virtualized?'live_memory_virtualized_read':'live_memory_direct',persistence_status:'QUOTA_UNAVAILABLE',persistence_error:String(error&&error.message||error),virtual_read_installed:virtualized}}}
...
function receipt(reason,status,extra){const live=liveReceipt();LAST_BOOT_RECEIPT=Object.assign({type:'PMP_CURRENT_BCD_DIAGNOSTICS_BOOTSTRAP_V1',version:V,classification_revision:CLASSIFICATION_REVISION,publication_revision:PUBLICATION_REVISION,owner:'diagnostics_owner',at:now(),reason:reason||'boot',status,required_version:REQUIRED_BCD_VERSION,versioned_source:VERSIONED_BCD_SRC,observed_api_version:currentApi()&&currentApi().version||null,observed_receipt_version:live&&live.version||null,complete_sections:REQUIRED_SECTIONS.filter(key=>live&&live[key]),transactional:true,bounded_publication:true,quota_resilient_live_publication:true,boundaries:{trace_ui:'retired',owner_changes:false,helper_changes:false,route_changes:false,storage_migration:false,persisted_user_data_write:false}},extra||{});store(BOOT_RECEIPT_KEY,LAST_BOOT_RECEIPT);return LAST_BOOT_RECEIPT}
