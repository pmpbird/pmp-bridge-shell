(()=>{
'use strict';
const V='1.1.0-level3-inside-source-zip-levels';
...
async function search(q){q=String(q||'').trim().toLowerCase();if(!q)return 'Type search words first.';let a=await all(),hits=[];for(const r of a){let t=String(r.text||''),lo=t.toLowerCase(),i=lo.indexOf(q);if(i>=0)hits.push(String(r.order_index).padStart(2,'0')+' '+r.note_id+' — '+small(t.slice(Math.max(0,i-80),i+160),240))}return hits.length?('Search hits: '+hits.length+'\n\n'+hits.join('\n\n')):'No hits.'}
async function openNote(q){q=String(q||'').trim().toLowerCase();if(!q)return 'Enter note number or id.';let a=await all();let r=a.find(x=>String(x.note_id).toLowerCase()===q||String(x.note_number).toLowerCase()===q||String(x.order_index)===q||String(x.order_index).padStart(2,'0')===q);if(!r)return 'Note not found.';return 'Note '+r.note_id+'\nFile: '+r.file_name+'\nChars: '+r.characters+'\n\n'+String(r.text||'').trim()}
