(()=>{
  ...
  function compileLocal(d,state,controller,engine){
    ...
    saveDraft(draft);renderDraft(d,draft);out(d,{status:'QUEUE_DRAFT_BUILT',meaning:'Free queued plan draft was built inside the app. Execution did not start.',next:'Review the queue. Enablement is a separate future step.',draft:draft},'hold');return false
  }
  function renderDraft(d,draft){
    ...
    p.innerHTML='';
    const title=d.createElement('div');title.style.fontWeight='950';title.textContent='Draft queue — not started';p.appendChild(title);
  ...
  async function openInline(w,d){
    ...
    const draft=loadDraft();renderDraft(d,draft);
    ...
    s.querySelector('[data-pmp-ap-preview]').onclick=e=>{e.preventDefault();let draft=loadDraft();renderDraft(d,draft);out(d,draft?{status:'QUEUE_PREVIEW',execution_started:false,draft:draft}:{status:'NO_DRAFT',execution_started:false},draft?'hold':'pass');return false};
