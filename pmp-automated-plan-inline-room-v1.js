(()=>{
  ...
  const SCREEN_ID='continuousRunDashboardV1';
  ...
  function recoverKept(){let kept=loadKept();if(!kept)return null;let draft=withRecoveryMeta(Object.assign({},kept,{keep_status:'recovered_from_kept_slot',status:'continuous_mission_not_started',execution_enabled:false,start_requested:false}));localStorage.setItem(DRAFT_KEY,JSON.stringify(draft,null,2));return draft}
  ...
  function summaryOut(d,draft){
    ...
    if(!draft){out(d,'No continuous mission draft yet.','pass');return}
    el.className='hold';el.innerHTML='';
    const title=d.createElement('div');title.style.cssText='font-size:20px;font-weight:950;margin-bottom:10px';title.textContent='Continuous Run Mission Set';el.appendChild(title);
    [['Status',draft.status||'continuous_mission_not_started'],['Mode',draft.mission_mode||'automatic pass-to-pass after future start'],['Execution',draft.execution_enabled?'enabled':'locked / not started'],['Start requested',draft.start_requested?'yes':'no'],['Resume from',draft.resume_from||'pass_003'],['Storage',draft.storage_mode==='kept_single_slot'?'kept single slot':'temporary recovery'],['Keep status',draft.keep_status||'not kept']].forEach(([k,v])=>el.appendChild(row(d,k,v)));
    const next=d.createElement('div');next.style.cssText='margin-top:12px;font-weight:900';next.textContent='This sets the standing mission only. A separate future Start Continuous Run gate is still required.';el.appendChild(next);
  }
  function rawOut(d,draft){out(d,draft?{status:'RAW_CONTINUOUS_MISSION_DRAFT',execution_started:false,draft:draft}:{status:'NO_DRAFT',execution_started:false},draft?'hold':'pass')}
  function storageOut(d){let draft=loadDraft(),kept=loadKept();out(d,{status:'DRAFT_STORAGE',execution_started:false,active_recovery_draft:draft?{exists:true,stored_at:draft.stored_at||null,expires_at:draft.recovery_expires_at||null,resume_from:draft.resume_from||null,mode:draft.mission_mode||null}:{exists:false},kept_draft:kept?{exists:true,kept_at:kept.kept_at||null,resume_from:kept.resume_from||null,mode:kept.mission_mode||null}:{exists:false},rule:'One temporary recovery mission draft. One optional kept mission draft. No long list.'},'hold')}
  function compileLocal(d,state,controller,engine){
    const box=d.getElementById('pmpApCommandBox');
    const mission=String(box&&box.value||'').trim();
    if(!mission){out(d,{status:'STOP',reason:'unclear_continuous_mission',message:'Describe the standing run mission first. Execution did not start.'},'fail');return false}
    if(mission.length>2400){out(d,{status:'STOP',reason:'mission_too_large',message:'Mission text is too large for the free in-app compiler. Execution did not start.'},'fail');return false}
    if(/\b(paid|billing|api key|merge now|force push|delete repo|secret)\b/i.test(mission)){out(d,{status:'STOP',reason:'hard_free_path_stop_gate',message:'Blocked paid/unsafe/secret wording. Execution did not start.'},'fail');return false}
    const cp=state&&state.checkpoint||{};const next=controller&&controller.next_unit||cp.next_unit||'pass_003';
    const draft={type:'PMP_FREE_IN_APP_CONTINUOUS_RUN_MISSION_DRAFT',schema_id:'pmp.automated-plan.continuous-run-mission.v1',schema_version:'1.0.0',status:'continuous_mission_not_started',dashboard_label:'Continuous Run Dashboard',mission_mode:'automatic_pass_to_pass_after_future_start',standing_mission:true,continuous_pass_to_pass_mission:true,auto_continue_after_verified_pass_after_enablement:true,requires_future_start_enablement:true,execution_enabled:false,start_requested:false,compiled_inside_app:true,compiled_at:new Date().toISOString(),mission_text:mission,resume_from:next,queue:[
      {unit_id:'mission_intake',objective:'Normalize the standing continuous-run mission and confirm it is free-path safe.'},
      {unit_id:'next_pass_scope',objective:'Begin at the current resume boundary and handle one verified pass at a time.'},
      {unit_id:'per_pass_compile_and_verify',objective:'Compile, run, and verify each pass only after future execution enablement.'},
      {unit_id:'automatic_continue_gate',objective:'After a pass verifies, continue to the next queued pass without asking again.'},
      {unit_id:'stop_gate_watch',objective:'Stop for safety gates, failed verification, checkpoint mismatch, paid path, write/merge approval, unclear instruction, or manual stop.'},
      {unit_id:'temporary_recovery_checkpoint',objective:'Keep one temporary recovery draft/checkpoint so an interrupted run can resume.'},
      {unit_id:'future_start_gate',objective:'Stay locked until a separate explicit Start Continuous Run action is added and approved.'}
  ...
  function renderDraft(d,draft){
    ...
    if(!draft){p.textContent='No continuous mission draft yet.';return}
    p.innerHTML='';
    const title=d.createElement('div');title.style.fontWeight='950';title.textContent='Continuous mission queue — not started';p.appendChild(title);
  ...
  async function openInline(w,d){
    ...
    s.innerHTML='<div class="card"><h1>Continuous Run Dashboard</h1><p class="sub" id="pmpApInlineStatus"></p><button class="big" data-pmp-ap-back="1"><span class="icon">←</span><span>Back to Control Room<small>return without changing plan state</small></span><span class="chev">›</span></button><div style="display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin:10px 0 0"><button class="mini" data-pmp-ap-refresh="1" style="width:100%;margin:0;min-width:0">Refresh</button><button class="mini" data-pmp-ap-details="1" style="width:100%;margin:0;min-width:0">Details</button></div><div class="note" id="pmpApInlineNote">This dashboard defines a standing Continuous Run Mission. Drafts are temporary recovery by default. Keep Draft saves one optional single-slot draft. Execution remains locked. Pass 003 has not started.</div><div class="panel hidden" id="pmpApInlineDetails" style="margin-top:10px"></div></div><div class="card"><h2>Continuous Run Mission</h2><p class="sub">Define the standing mission for automatic pass-to-pass running. This does not start the run.</p><textarea id="pmpApCommandBox" style="min-height:140px" placeholder="Run continuously through the queued passes, one verified pass at a time, using only the free path. After each pass finishes and verifies, continue to the next pass automatically. Stop only for safety gates, failed verification, unclear instructions, checkpoint mismatch, paid path, write/merge approval, or manual stop."></textarea><div class="grid"><button class="mini" data-pmp-ap-compile="1">Set Continuous Run Mission</button><button class="mini" data-pmp-ap-preview="1">Preview Mission Queue</button><button class="mini" data-pmp-ap-raw="1">Raw Draft</button><button class="mini" data-pmp-ap-keep="1">Keep Draft</button><button class="mini" data-pmp-ap-recover="1">Recover Kept</button><button class="mini" data-pmp-ap-storage="1">Draft Storage</button><button class="mini" data-pmp-ap-clear="1">Clear Temporary</button><button class="mini" data-pmp-ap-clearkept="1">Clear Kept</button><button class="mini" data-pmp-ap-gates="1">Show Stop Gates</button></div><div id="pmpApEngineOut" class="pass">Ready. Continuous run is not started.</div><div id="pmpApQueuePreview" class="note" style="margin-top:10px">No continuous mission draft yet.</div></div><div class="card"><h2>Hard Free-Path Gates</h2><div class="hold">Free only. No paid API. No paid fallback. $0 ceiling. No write authority. No merge authority. Stop on ambiguity, changed main, checkpoint mismatch, unsafe path, failed verification, or manual stop.</div></div>';
    wrap.appendChild(s);
    Array.from(d.querySelectorAll('.screen')).forEach(x=>{if(x!==s)x.classList.remove('on')});
    d.location.hash='#continuous-run-dashboard';
