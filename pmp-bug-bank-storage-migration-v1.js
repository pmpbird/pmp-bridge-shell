(()=>{
'use strict';
const V='1.0.2-keep-variants-group-later';
...
function compactDetail(x){try{return normText(JSON.stringify(x.detail||x.raw_source||{}).slice(0,260))}catch(e){return''}}
function exactKey(x){return [normText(x.type||x.kind||x.problem),normText(x.where||x.where_seen),normText(x.how_it_happens||x.why||x.symptom),normText(x.fix_rule),compactDetail(x)].join('|')||normText(JSON.stringify(x).slice(0,220))}
function merge(){let out=[],seen={};Array.from(arguments).forEach(list=>arr(list).forEach(x=>{let k=exactKey(x);if(seen[k]){let old=seen[k];Object.assign(old,x,{seen:Math.max(Number(old.seen||1),Number(x.seen||1)),last_seen:x.last_seen||old.last_seen})}else{seen[k]=Object.assign({},x);out.push(seen[k])}}));return out}
