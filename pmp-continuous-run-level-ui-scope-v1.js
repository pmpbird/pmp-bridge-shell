(()=>{
'use strict';
const V='2.0.0-pass10-unit7-single-owner-stable-order-20260726A';
...
function denestAndOrder(d){let b=bank(d),h=ensureHost(d),moved=0,hidden=0,created=0,dupes=0;if(!b||!h)return{moved,hidden,created,dupes};let s=stack(h),ready=readiness(b),cands=collectCandidates(b,h);LEVELS.forEach(meta=>{let ensured=ensureShell(d,s,meta,ready),sh=ensured.shell,content=sh.querySelector('[data-cr-level-content-v1]');if(ensured.moved)moved++;dupes+=ensured.deduped;let list=cands.filter(c=>c.id===meta.id&&c.el!==sh&&c.el!==content&&!(c.el.contains&&c.el.contains(sh)));let chosen=list.find(c=>properShell(c.el,meta.id))||list.find(c=>!isOwnedByThis(c.el))||null;if(chosen&&content){Array.from(content.querySelectorAll('[data-cr-level-placeholder-text-v1],[data-cr-level-placeholder-status-v1]')).forEach(x=>{if(x.style.display!=='none')x.style.display='none'});if(chosen.el.parentElement!==content){content.appendChild(chosen.el);moved++}styleBase(chosen.el);S(chosen.el,'display','block');S(chosen.el,'visibility','visible');try{chosen.el.setAttribute('data-cr-level-canonical-member-v1',meta.id)}catch(e){}}else{created++;Array.from(content.querySelectorAll('[data-cr-level-placeholder-text-v1],[data-cr-level-placeholder-status-v1]')).forEach(x=>{if(x.style.display)x.style.display=''})}
...
function scanDoc(d){if(!isCR(d))return null;scans++;let r=denestAndOrder(d),changed=r.moved+r.hidden+r.dupes;if(changed){totalMoves+=changed}else noops++;return put(RECEIPT,{type:'PMP_CONTINUOUS_RUN_LEVEL_UI_SCOPE_V1',version:V,at:now(),status:'CANONICAL_LEVEL3PLUS_ORDER_READY',levels_visible:'3_through_30B',readiness_state:r.readiness,real_level_nodes_moved:r.moved,placeholder_level_cards_active:r.created,loose_level_nodes_contained:r.hidden,duplicate_level_nodes_contained:r.dupes,canonical_order:LEVELS.map(x=>x.id),scan_mode:'EVENT_DRIVEN_EXTERNAL_CHILD_ADDITIONS_ONLY',scan_count:scans,no_op_scan_count:noops,total_dom_corrections:totalMoves,external_mutation_signals:externalSignals,recurring_timer_active:false,rule:'Level 3+ is visible exactly once in canonical order inside the one Continuous Run Owner stack. A no-op scan preserves node identity and DOM order. Bank home can never own or display these nodes.',side_effects:{route_change:'not_attempted',bank_rebuild:'not_attempted',indexeddb_write:'not_attempted',storage_migration:'not_attempted',broad_dom_sweep:'not_attempted',recurring_reparent_loop:'disabled',hide_reason:'only loose_or_duplicate_level_nodes_outside_canonical_stack'}})}
function levelSignal(node){try{if(!node||node.nodeType!==1)return false;if(node.closest&&node.closest('[data-continuous-run-level-ui-scope-v1]'))return false;if(node.matches&&node.matches(ALL_SELECTORS))return true;if(node.querySelector&&node.querySelector(ALL_SELECTORS))return true;return /^Level\s+(30B|4B|[1-9]|[12][0-9]|30)\b/i.test(clean(node.textContent||''))}catch(e){return false}}
