(()=>{
'use strict';
const V='2.1.0-pass10-unit7-legacy-alias-single-stack-20260727A';
...
const LEVELS=[
 ...
 ['19','[data-level19-recovery-proof],[data-level19-recert-recovery]','Level 19 — Recovery Proof'],
 ['20','[data-level20-summary-lock]','Level 20 — Summary Lock'],
 ['21','[data-level21-one-tap-retest],[data-level21-full-retest]','Level 21 — One-Tap Re-Test Lock'],
 ['22','[data-level22-export-receipt]','Level 22 — Export Receipt Integrity Check'],
 ['23','[data-level23-integrity-check],[data-level23-integrity]','Level 23 — Receipt Bundle Integrity Check'],
 ['24','[data-level24-tamper-test]','Level 24 — Tampered Receipt Rejection Test'],
 ['25','[data-level25-receipt-bundle],[data-level25-bundle],[data-level25-packet]','Level 25 — Receipt Bundle'],
 ['26','[data-level26-packet-integrity]','Level 26 — Portable Packet Integrity Check'],
 ['27','[data-level27-tampered-packet],[data-level27-tamper-packet]','Level 27 — Tampered Packet Rejection Test'],
...
const LEGACY_BADGE_SELECTOR=LEVELS.filter(x=>x.id!=='3'&&x.id!=='4').map(x=>'[data-l'+x.id.toLowerCase()+'-badge]').join(',');
const WATCHED=new WeakMap();
let scans=0,noops=0,totalMoves=0,externalSignals=0;
...
function S(x,k,v){try{if(x.style.getPropertyValue(k)===v&&x.style.getPropertyPriority(k)==='important')return false;x.style.setProperty(k,v,'important');return true}catch(e){return false}}
...
function ignore(el){return !el||!el.parentNode||(el.hasAttribute&&el.hasAttribute('data-cr-level-shell-v1'))||(el.hasAttribute&&el.hasAttribute('data-cr-level-content-v1'))||(el.hasAttribute&&el.hasAttribute('data-cr-level-legacy-mount-sentinel-v1'))||!!(el.closest&&el.closest('[data-cr-level-head]'))}
function selectorAttrs(meta){return Array.from(meta.selector.matchAll(/\[([a-zA-Z0-9_-]+)\]/g),m=>m[1])}
function legacyBadgeId(el){try{for(const a of Array.from(el.attributes||[])){let m=a.name.match(/^data-l(4b|[0-9]+)-badge$/i);if(m&&BY_ID[m[1].toUpperCase()])return m[1].toUpperCase()}}catch(e){}return null}
function ensureLegacyMountSentinel(root,meta){if(!root||meta.id==='4')return false;let q='[data-cr-level-legacy-mount-sentinel-v1="'+meta.id+'"]',sentinel=root.querySelector(q),created=false;if(!sentinel){sentinel=root.ownerDocument.createElement('span');sentinel.setAttribute('data-cr-level-legacy-mount-sentinel-v1',meta.id);sentinel.setAttribute('aria-hidden','true');selectorAttrs(meta).forEach(a=>sentinel.setAttribute(a,''));root.appendChild(sentinel);created=true}S(sentinel,'display','none');S(sentinel,'visibility','hidden');S(sentinel,'pointer-events','none');return created}
function hideLegacyBadges(b){let hidden=0;if(!b||!LEGACY_BADGE_SELECTOR)return hidden;try{Array.from(b.querySelectorAll(LEGACY_BADGE_SELECTOR)).forEach(x=>{let was=x.style.getPropertyValue('display')==='none'&&x.style.getPropertyPriority('display')==='important';x.setAttribute('data-cr-level-legacy-badge-contained-v1',legacyBadgeId(x)||'unknown');x.setAttribute('aria-hidden','true');S(x,'display','none');S(x,'visibility','hidden');S(x,'pointer-events','none');if(!was)hidden++})}catch(e){}return hidden}
...
function ensureShell(d,s,meta,ready){let shells=Array.from(s.querySelectorAll('[data-cr-level-shell-v1][data-cr-level-id="'+meta.id+'"]')),sh=shells.shift()||null,moved=false,deduped=0;shells.forEach(x=>{x.remove();deduped++;moved=true});if(!sh){sh=d.createElement('section');sh.setAttribute('data-cr-level-shell-v1','');sh.setAttribute('data-cr-level-id',meta.id);sh.setAttribute('data-cr-level-order',String(meta.index+3));sh.innerHTML='<div data-cr-level-readiness-badge-v1></div><h3>'+esc(meta.title)+'</h3><div data-cr-level-content-v1><p class="sub" data-cr-level-placeholder-text-v1>'+esc(ready.note)+'</p><pre class="note" data-cr-level-placeholder-status-v1>Readiness: '+esc(ready.state)+'\nOrder: visible inside Continuous Run Owner stack\nRule: visible does not mean runnable/trusted yet.</pre></div>';moved=true}let at=s.children[meta.index]||null;if(at!==sh){s.insertBefore(sh,at);moved=true}sh.setAttribute('data-cr-level-readiness-state',ready.state);let badge=sh.querySelector('[data-cr-level-readiness-badge-v1]');if(badge&&badge.textContent!==ready.label)badge.textContent=ready.label;styleBase(sh);if(badge){S(badge,'display','inline-block');S(badge,'width','fit-content');S(badge,'margin','0 0 8px 0');S(badge,'padding','6px 10px');S(badge,'border','2px solid rgba(0,0,0,.35)');S(badge,'border-radius','999px');S(badge,'font-weight','950');S(badge,'background','#fff3de')}return{shell:sh,moved,deduped}}
...
function denestAndOrder(d){let b=bank(d),h=ensureHost(d),moved=0,hidden=0,created=0,dupes=0,sentinels=0,legacyBadgesHidden=0;if(!b||!h)return{moved,hidden,created,dupes,sentinels,legacyBadgesHidden};let s=stack(h),ready=readiness(b),cands=collectCandidates(b,h);LEVELS.forEach(meta=>{let ensured=ensureShell(d,s,meta,ready),sh=ensured.shell,content=sh.querySelector('[data-cr-level-content-v1]');if(ensured.moved)moved++;dupes+=ensured.deduped;let list=cands.filter(c=>c.id===meta.id&&c.el!==sh&&c.el!==content&&!(c.el.contains&&c.el.contains(sh)));let chosen=list.find(c=>properShell(c.el,meta.id))||list.find(c=>!isOwnedByThis(c.el))||list.find(c=>{let own=c.el.closest&&c.el.closest('[data-cr-level-shell-v1]');return own&&own.getAttribute('data-cr-level-id')!==meta.id})||null;if(chosen&&content){Array.from(content.querySelectorAll('[data-cr-level-placeholder-text-v1],[data-cr-level-placeholder-status-v1]')).forEach(x=>{if(x.style.display!=='none')x.style.display='none'});let legacyRoot=chosen.el.closest&&chosen.el.closest('[data-source-reference-gate-level4]');if(!legacyRoot&&meta.id!=='3'&&meta.id!=='4')legacyRoot=b.querySelector('[data-source-reference-gate-level4]');if(chosen.el.parentElement!==content){content.appendChild(chosen.el);moved++}if(legacyRoot&&legacyRoot!==chosen.el&&ensureLegacyMountSentinel(legacyRoot,meta))sentinels++;styleBase(chosen.el);S(chosen.el,'display','block');S(chosen.el,'visibility','visible');try{chosen.el.setAttribute('data-cr-level-canonical-member-v1',meta.id)}catch(e){}}else{created++;Array.from(content.querySelectorAll('[data-cr-level-placeholder-text-v1],[data-cr-level-placeholder-status-v1]')).forEach(x=>{if(x.style.display)x.style.display=''})}
 ...
 legacyBadgesHidden+=hideLegacyBadges(b);
 return{moved,hidden,created,dupes,sentinels,legacyBadgesHidden,readiness:ready.state}}
function scanDoc(d){if(!isCR(d))return null;scans++;let r=denestAndOrder(d),changed=r.moved+r.hidden+r.dupes+r.sentinels+r.legacyBadgesHidden;if(changed){totalMoves+=changed}else noops++;return put(RECEIPT,{type:'PMP_CONTINUOUS_RUN_LEVEL_UI_SCOPE_V1',version:V,at:now(),status:'CANONICAL_LEVEL3PLUS_ORDER_READY',levels_visible:'3_through_30B',readiness_state:r.readiness,readiness_badge_contract:'PASS9_FAIL_CLOSED_VISIBLE_NOT_RUNNABLE_UNTIL_PROOF',real_level_nodes_moved:r.moved,legacy_mount_sentinels_created:r.sentinels,legacy_level_badges_contained:r.legacyBadgesHidden,placeholder_level_cards_active:r.created,loose_level_nodes_contained:r.hidden,duplicate_level_nodes_contained:r.dupes,canonical_order:LEVELS.map(x=>x.id),legacy_aliases_contained:['19','21','23','25','27'],scan_mode:'EVENT_DRIVEN_EXTERNAL_CHILD_ADDITIONS_ONLY',scan_count:scans,no_op_scan_count:noops,total_dom_corrections:totalMoves,external_mutation_signals:externalSignals,recurring_timer_active:false,rule:'Level 3+ is visible exactly once in canonical order inside the one Continuous Run Owner stack. Legacy Level 4 injectors retain hidden mount sentinels so they cannot recreate duplicate controls. Locked / waiting is an intentional fail-closed readiness state, not a second level. A no-op scan preserves node identity and DOM order. Bank home can never own or display these nodes.',side_effects:{route_change:'not_attempted',bank_rebuild:'not_attempted',indexeddb_write:'not_attempted',storage_migration:'not_attempted',broad_dom_sweep:'not_attempted',recurring_reparent_loop:'disabled',hide_reason:'only loose_or_duplicate_level_nodes_and_redundant_legacy_badges'}})}
function signalId(el){if(!el||el.nodeType!==1||(el.hasAttribute&&el.hasAttribute('data-cr-level-legacy-mount-sentinel-v1')))return null;let id=levelId(el)||legacyBadgeId(el);if(id)return id;try{let found=el.querySelector(ALL_SELECTORS+(LEGACY_BADGE_SELECTOR?','+LEGACY_BADGE_SELECTOR:''));return found?(levelId(found)||legacyBadgeId(found)):null}catch(e){return null}}
function levelSignal(node){try{if(!node||node.nodeType!==1)return false;let id=signalId(node);if(!id)return /^Level\s+(30B|4B|[1-9]|[12][0-9]|30)\b/i.test(clean(node.textContent||''));let scope=node.closest&&node.closest('[data-continuous-run-level-ui-scope-v1]');if(!scope)return true;let shell=node.closest&&node.closest('[data-cr-level-shell-v1]');return !!(shell&&shell.getAttribute('data-cr-level-id')!==id)}catch(e){return false}}
function watch(d){try{let b=bank(d);if(!b||WATCHED.has(d)||typeof MutationObserver!=='function')return;let pending=false;let observer=new MutationObserver(rows=>{if(!rows.some(row=>Array.from(row.addedNodes||[]).some(levelSignal)))return;externalSignals++;if(pending)return;pending=true;setTimeout(()=>{pending=false;try{scanDoc(d)}catch(e){}},0)});observer.observe(b,{childList:true,subtree:true});WATCHED.set(d,observer)}catch(e){}}
...
window.addEventListener('pmp:continuous-run-level-source-ready',scan);
scan();
})();
