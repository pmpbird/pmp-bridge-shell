(()=>{
'use strict';
const V='1.0.4-fast-zero-bank-loading-flash-guard';
const SCRIPTS=[
  {id:'pmpBankZeroLoadingFlashGuardV1DirectFrame',src:'pmp-bank-zero-loading-flash-guard-v1.js',fresh:'zero-loading-flash-guard-v101-20260629B'},
...
function scan(){let made=0;docs(document).forEach(d=>{made+=inject(d)});try{localStorage.setItem('pmp_continuous_run_bank_order_frame_loader_v1_receipt',JSON.stringify({type:'PMP_CONTINUOUS_RUN_BANK_ORDER_FRAME_LOADER_V1',version:V,at:now(),scripts:SCRIPTS.map(x=>x.src),new_injections:made,note:'Fast-injects zero-loading-flash guard before live Bank shell can paint loading text; no route/storage/engine/core logic change.'},null,2))}catch(e){}}
window.PMPContinuousRunBankOrderFrameLoaderV1={version:V,scan};
window.addEventListener('load',()=>[0,10,25,50,80,120,180,250,350,500,800,1200,1800,2600,3600].forEach(t=>setTimeout(scan,t)));
let fast=setInterval(scan,25);setTimeout(()=>{try{clearInterval(fast)}catch(e){};setInterval(scan,250)},7000);
scan();
})();