...
function loadTrace(){try{return JSON.parse(localStorage.getItem(TRACE_KEY)||'null')||{type:'PMP_PASS7_STARTUP_EXECUTION_AUDIT_TRACE_V1',version:V,owner:'pmp-continuous-run-bank-order-frame-loader-v1',created_at:now(),events:[]}}catch(e){return{type:'PMP_PASS7_STARTUP_EXECUTION_AUDIT_TRACE_V1',version:V,owner:'pmp-continuous-run-bank-order-frame-loader-v1',created_at:now(),events:[{at:now(),event:'trace_parse_error',error:String(e&&e.message||e)}]}}}
...
window.addEventListener('load',()=>[0,25,50,100,200,400,900,1800,3200,5200,8000].forEach(t=>setTimeout(()=>scan('load_'+t),t)));
setInterval(()=>scan('interval_600'),600);
scan('initial');
})();